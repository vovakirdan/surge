use surge_token::{SourceId, Span, Token, TokenKind, lookup_keyword};
...
impl<'a> Lexer<'a> {
    ...
    fn next_token(&mut self) {
        ...
        let (kind, len) = match b {
            ...
            b'<' => (TokenKind::LAngle, 1),
            b'>' => (TokenKind::RAngle, 1),
            b'|' => (TokenKind::Pipe, 1),
            b',' => (TokenKind::Comma, 1),
            ...
            _ => {
                self.lex_word_or_number_or_string();
                return;
            }
        };
        self.push(kind, len);
    ...
    fn take_multi<'s>(&self, ks: &[&'s str]) -> Option<&'s str> {
        for &k in ks {
            if self.src[self.i..].starts_with(k) {
                return Some(k);
            }
        }
        None
