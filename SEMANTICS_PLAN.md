...
**Статус:** ✅ Готово. Возвраты и аннотации `-> T` контролируются (`validateReturn`), параметры получают типы и попадают в `bindingTypes`. Магические методы/интринсики `extern<T>` доступны через таблицу `magic`. Сахар `?`/`try` по дизайну убран; автозаворачивание в `Some/Ok` остаётся как есть.
