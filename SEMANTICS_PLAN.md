...
**Статус:** ⚠️ Частично. Возвраты и аннотации `-> T` контролируются (`validateReturn`), параметры получают типы и попадают в `bindingTypes`. Магические методы/интринсики `extern<T>` доступны через таблицу `magic`. Пока нет поддержки `?`/`try`-сахара и статического диспатча общих `extern` наборов вне магических методов — call-resolver (#6) всё ещё отсутствует.
...
**Статус:** ⛔️ Не сделано. Вызовы пока лишь типизируют аргументы, но не выбирают сигнатуры (`callResultType` возвращает `NoTypeID`). Нет алгоритма стоимости приведения, не учитываются вариадики и союзы, отсутствуют диагностики неоднозначности/нет подходящих перегрузок.
...
**Статус:** ⚠️ Частично. `type_expr.go` и `magic_names.go` поддерживают `expr to Type` и `__to`-методы, встроенные числовые касты заведены в `magic_builtins`. Но `__cast` протокол ещё не реализован, нет проверки конфликтов, отсутствуют специализированные диагностические коды (`пока используем SemaTypeMismatch`), не запрещены опасные касты ссылок/указателей.
...
7. ⚠️ `to` работает через `__to`/intrinsics, но `__cast` и спец-ошибки не внедрены. 
