...
func untypedLiteralCases() []untypedLiteralCase {
    ...
    return take(p) + take(p);
		...
		{name: "method_argument", src: `fn f(b: &mut Bag) { b.put({ x: 1, y: 2 }); }`,
			refused: []string{"{ x: 1, y: 2 }"}},
		// A method that does not exist is reported as such, and covers the literal it was given.
		{name: "missing_method_with_literal_argument", src: `fn f(b: &Bag) { b.nope({ x: 1, y: 2 }); }`,
			other: "SEM3005"},
