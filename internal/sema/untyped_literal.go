...
// resolveLetAnnotation resolves a simple `let`'s annotation. When the annotation
// names no type and resolving it reported an error, that error explains the
// literal the `let` binds, which then gets no SemaLiteralNeedsType of its own.
func (tc *typeChecker) resolveLetAnnotation(letStmt *ast.LetStmt, scope symbols.ScopeID) types.TypeID {
	checkpoint := tc.errorCheckpoint()
	declared := tc.resolveTypeExprWithScope(letStmt.Type, scope)
	if letStmt.Type.IsValid() && declared == types.NoTypeID && tc.hasErrorsSince(checkpoint) {
