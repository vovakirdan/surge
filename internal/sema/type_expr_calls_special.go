...
func (tc *typeChecker) handleCloneCall(callID ast.ExprID, args []callArg, span source.Span) types.TypeID {
	...
	// The implementation is chosen once for the whole program, after every
	// module has been merged: a type must clone the same way everywhere, and the
	// canonical body may live in a module this file only imports. All this file
	// contributes is the borrow obligation and the request itself.
	if args[0].expr.IsValid() {
		tc.applyParamOwnership(symbols.TypeKey("&"), args[0].expr, args[0].ty, tc.exprSpan(args[0].expr))
	}
	tc.recordDirectCloneRequest(&DirectCloneRequest{
		Use:          callID,
		Owner:        tc.currentFnSym(),
		Receiver:     innerType,
		AccessModule: tc.modulePath,
		Site:         span,
		TypeLabel:    tc.typeLabel(innerType),
	})
	return innerType
}
