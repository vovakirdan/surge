...
var crossingCodeDescriptions = map[Code]string{
	...
	SemaOnTcpRemoteIO:      "remote socket I/O through `far TcpConn` is not accepted in Epic 11",
	...
	SynCrossesFnType:             "function-type syntax for `crosses` is not part of Epic 11",

	FutFarArrayPostponed:               "`far` array handles (`far T[]`) are postponed",
	FutFarLocalArrayPostponed:          "local arrays of `far` handles need an accepted grouping syntax",
	FutFarFnHandle:                     "`far fn(...) -> T` remote function handles are not part of Epic 11",
	FutOnDestBlocking:                  "`on blocking` is not part of Epic 11",
	FutSpawnOnDestBlocking:             "`spawn on blocking` is not part of Epic 11",
