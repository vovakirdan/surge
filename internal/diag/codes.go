...
var ( // todo расширить описания и использовать как notes
	codeDescription = map[Code]string{
		...
		// explicit-crossing diagnostics (codes_crossing.go)
		...
		// clone-canonicality diagnostics (codes_clone.go)
		...
		// entrypoint diagnostics (codes_entrypoint.go)
		...
		// alias magic-method diagnostics (codes_alias_magic.go)
		SemaAliasMagicRedeclared: "an alias cannot declare a magic method its target declares",

		// physical-layout diagnostics (codes_layout.go)
		SemaLayoutOverflow:             "physical layout exceeds the target address space",
		SemaLayoutUnsupportedAlignment: "alignment is not supported by the target ABI",
		SemaLayoutDeferred:             "physical layout is not concrete after type checking",
	}
)
