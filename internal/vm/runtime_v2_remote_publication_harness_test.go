...
// The C stand the remote-publication tests drive, kept apart from the
// questions they ask of it.
//
// It is one program with a mode switch rather than one per case, because every
// case needs the same runtime brought up the same way; splitting it per test
// would duplicate the setup and let the copies drift.
