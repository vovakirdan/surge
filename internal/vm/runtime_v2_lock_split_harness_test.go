...
const lockSplitHarnessPrologue = `
...
static rt_task* alloc_ready_task(rt_executor* ex, int64_t poll_fn_id) {
    ...
    (void)rt_value_cell_bind(&task->result, rt_channel_opaque_word_ops());  // answers with a word
    task->id = id;
...
// main() and its mode switch, last so the modes it names are all defined.
const lockSplitHarnessMain = `
