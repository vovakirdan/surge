...
func TestVMTuplesGolden(t *testing.T) {
	...
	tuplesDir := filepath.Join(root, "testdata", "golden", "vm_tuples")
	entries, err := os.ReadDir(tuplesDir)
	if err != nil {
		t.Fatalf("read vm_tuples dir: %v", err)
	}
	...
	for _, ent := range entries {
		...
		t.Run(name, func(t *testing.T) {
			sgRel := filepath.ToSlash(filepath.Join("testdata", "golden", "vm_tuples", name+".sg"))
			outAbs := filepath.Join(tuplesDir, name+".out")
			codeAbs := filepath.Join(tuplesDir, name+".code")
