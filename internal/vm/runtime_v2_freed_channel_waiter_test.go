...
// frees the channel.
//
// The window is SP_WAKE_BEFORE_STALE_REMOVAL, which already existed; the stand
// arms it itself once its setup is over (rt_sync_point_arm_block). The row is a
// pair, never a single green run: the negative-control build restores the
...
func runFreedChannelWaiterHarness(t *testing.T, bin string) (string, string, int) {
	...
	// The window is NOT armed from the environment: a process-wide arming holds
	// the first thread to reach it, and a setup-phase wake (the mint's reply,
	// in one schedule) would leave the stand's unbounded await hanging. The
	// stand arms the point itself, after its setup, and disarms it after the
	// one reach it holds -- see rt_sync_point_arm_block.
