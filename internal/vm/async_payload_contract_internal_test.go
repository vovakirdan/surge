...
func TestAsyncPayloadAPIRequiresCallerOwnedDestination(t *testing.T) {
	packages, err := parseProductionVMPackage()
	if err != nil {
		t.Fatal(err)
	}
	foundMove := false
...
func parseProductionVMPackage() (map[string]*ast.Package, error) {
	return parser.ParseDir(token.NewFileSet(), ".", func(info os.FileInfo) bool {
