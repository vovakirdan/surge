...
func TestVMStringsGolden(t *testing.T) {
	...
	stringsDir := filepath.Join(root, "testdata", "golden", "vm_strings")
	entries, err := os.ReadDir(stringsDir)
	if err != nil {
		t.Fatalf("read vm_strings dir: %v", err)
	}
	...
	for _, ent := range entries {
		...
		t.Run(name, func(t *testing.T) {
			sgRel := filepath.ToSlash(filepath.Join("testdata", "golden", "vm_strings", name+".sg"))
			outAbs := filepath.Join(stringsDir, name+".out")
			codeAbs := filepath.Join(stringsDir, name+".code")
