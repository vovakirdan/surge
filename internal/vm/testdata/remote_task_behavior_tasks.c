...
static void poll_child(task9_child_state* child) {
...
static void poll_publisher(task9_publish_state* state) {
...
static void poll_lifecycle(task9_lifecycle_state* state) {
    uint8_t kind = 0;
    uint64_t bits = 0;
...
void __surge_poll_call(uint64_t id) {
    ...
    if (id == POLL_TASK9_PUBLISHER) {
        poll_publisher((task9_publish_state*)__task_state());
    }
...
rt_far_task_handle* task9_publish_handle(task9_child_state* child, uint32_t destination) {
    return task9_publish_poll(POLL_TASK9_CHILD, child, destination);
}

rt_far_task_handle* task9_publish_poll(uint64_t poll_id, void* task_state, uint32_t destination) {
    ...
    void* task = __task_create(POLL_TASK9_PUBLISHER, &state);
    uint8_t kind = 0;
    uint64_t bits = 0;
    if (!task9_await(task, &kind, &bits) || state.status != RT_REMOTE_SPAWN_STATUS_OK) {
        return NULL;
    }
...
void* task9_start_lifecycle(task9_lifecycle_state* state, rt_far_task_handle* handle, int cancel) {
    ...
    void* task = __task_create(POLL_TASK9_LIFECYCLE, state);
