...
async fn run() -> int {
    ...
    //
    // The third is why the edge went from two units to one, in both verticals
    // at once and with the per-iteration growth untouched: a child's scope
    // membership stopped being decided by reading the child's status, so a
    // child that completes before its registration arrives is no longer
    // counted into the scope and the scope no longer holds the one reference
    // it used to hold for it. That happens once per window, not once per
    // .share(), which is exactly the shape of the change — both baselines fell
    // by one, both totals fell by one, and 4 and 5 per iteration stand.
    //
