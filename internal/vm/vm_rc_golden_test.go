...
func TestVMRCGolden(t *testing.T) {
	...
	rcDir := filepath.Join(root, "testdata", "golden", "vm_rc")
	entries, err := os.ReadDir(rcDir)
	if err != nil {
		t.Fatalf("read vm_rc dir: %v", err)
	}
	...
	for _, ent := range entries {
		...
		t.Run(name, func(t *testing.T) {
			sgRel := filepath.ToSlash(filepath.Join("testdata", "golden", "vm_rc", name+".sg"))
			outAbs := filepath.Join(rcDir, name+".out")
			codeAbs := filepath.Join(rcDir, name+".code")
