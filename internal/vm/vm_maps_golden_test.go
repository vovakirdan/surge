...
func TestVMMapsGolden(t *testing.T) {
	...
	mapsDir := filepath.Join(root, "testdata", "golden", "vm_maps")
	entries, err := os.ReadDir(mapsDir)
	if err != nil {
		t.Fatalf("read vm_maps dir: %v", err)
	}
	...
	for _, ent := range entries {
		...
		t.Run(name, func(t *testing.T) {
			sgRel := filepath.ToSlash(filepath.Join("testdata", "golden", "vm_maps", name+".sg"))
			outAbs := filepath.Join(mapsDir, name+".out")
			codeAbs := filepath.Join(mapsDir, name+".code")
