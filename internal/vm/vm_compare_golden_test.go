...
func TestVMCompareGolden(t *testing.T) {
	...
	compareDir := filepath.Join(root, "testdata", "golden", "vm_compare")
	entries, err := os.ReadDir(compareDir)
	if err != nil {
		t.Fatalf("read vm_compare dir: %v", err)
	}
	...
	for _, ent := range entries {
		...
		t.Run(name, func(t *testing.T) {
			sgRel := filepath.ToSlash(filepath.Join("testdata", "golden", "vm_compare", name+".sg"))
			outAbs := filepath.Join(compareDir, name+".out")
			codeAbs := filepath.Join(compareDir, name+".code")
