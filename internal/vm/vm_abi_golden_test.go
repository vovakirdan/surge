...
func TestVMABIGolden(t *testing.T) {
	...
	abiDir := filepath.Join(root, "testdata", "golden", "abi")
	entries, err := os.ReadDir(abiDir)
	if err != nil {
		t.Fatalf("read abi dir: %v", err)
	}
	...
	for _, ent := range entries {
		...
		t.Run(name, func(t *testing.T) {
			sgRel := filepath.ToSlash(filepath.Join("testdata", "golden", "abi", name+".sg"))
			outAbs := filepath.Join(abiDir, name+".out")
			codeAbs := filepath.Join(abiDir, name+".code")
