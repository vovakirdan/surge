...
func (l *funcLowerer) acquireGeneratedLoopValue(op *Operand, kind hir.GeneratedDropKind) Operand {
	acquired := *op
	if kind == hir.GeneratedDropCountedScalar && acquired.Kind == OperandCopy && l.isRefCountedScalar(acquired.Type) {
		acquired.Kind = OperandRetain
	}
	return acquired
}
...
func (l *funcLowerer) isNumericIterableResource(ty types.TypeID) bool {
	...
	if elem, isArray := l.types.ArrayInfo(ty); isArray {
		return l.isRefCountedScalar(elem)
	}
	if elem, _, isFixed := l.types.ArrayFixedInfo(ty); isFixed {
		return l.isRefCountedScalar(elem)
	}
