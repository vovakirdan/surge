...
func (l *funcLowerer) lowerSpawnOnPollFunc(id FuncID, name string, body *hir.Block, result, stateType types.TypeID, captures []spawnOnCaptureInfo, span source.Span) (*Func, error) {
	...
	for _, cap := range captures {
		...
		// An owned capture transfers from the frame into the body. A Copy
		// capture that owns heap transfers the private holder acquired at
		// construction; the caller still owns its original binding. The frame
		// is emptied at this read, and the local owes that holder's release.
		// A heap-free Copy capture has no ownership to transfer.
		l.emit(&Instr{Kind: InstrAssign, Assign: AssignInstr{
			...
			Src: RValue{Kind: RValueField, Field: FieldAccess{
				...
				MoveOut:   !cap.CopyCapture || l.ownsHeap(cap.Type),
			}},
		}})
