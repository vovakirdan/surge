...
// N-TAGCONV: the finalized use of an implicit Some/Success wrap (a tag use with no
...
// The other "disagrees" forms keep their rows (N-ALIAS-ARG, N-TAGMEMBER-ARG).
func TestReturnOriginTagConversionOtherFormsKeepTheirRows(t *testing.T) {
	rows := []struct{ name, text, snippet, reason string }{
		{"alias_argument", `type Nodes = int[];

fn count(nodes: Nodes) -> uint {
    return len(nodes);
}
`, "len(nodes)", "generic original call argument disagrees with its substituted source signature"},
