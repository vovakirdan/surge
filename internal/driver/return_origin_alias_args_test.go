...
// A generic call argument whose type differs from the substituted
...
func TestReturnOriginAliasArgumentCanariesKeepTheirRows(t *testing.T) {
	...
	rows := []struct {
		...
		{name: "window_pushed_into_container", text: `fn stash(out: &mut int[][]) -> nothing {
...
`, want: []want{{"out.push(xs[[1..3]])", aliasArgLoanDiscarded}}},
		{
			// The alias spelling matches its formal, so the loan-discard refusal
			// is the one row that holds this window.
			name: "window_pushed_into_alias_container", text: `type Win = int[];
...
`, want: []want{{"out.push(xs[[1..3]])", aliasArgLoanDiscarded}},
		},
		{
			// The alias spelling matches its formal, so the loan-discard refusal
			// is the one row that holds this window.
			name: "window_pushed_through_generic_alias", text: `type Win = int[];
...
`, want: []want{{"keep::<Win>(out, xs[[1..3]])", aliasArgLoanDiscarded}},
		...
		{name: "window_written_through_alias_reference", text: `type Win = int[];
...
`, want: []want{{"*out = xs[[1..3]]", aliasArgLoanDiscarded}}},
		{name: "window_sent_on_alias_channel", text: `type Win = int[];
...
// Escapes refused with SEM3139: a slice of a window returned, with and without an
// alias, and an alias to a reference passed through a generic
...
func TestReturnOriginAliasArgumentEscapesAreRefused(t *testing.T) {
	rows := []struct{ name, text string }{
		{"slice_of_window_returned", `fn view2() -> int[] {
    let a: int[4] = [1, 2, 3, 4];
    let v = a[[0..3]];
    return v[[0..1]];
}
`},
		{"slice_of_alias_window_returned", `type Win = int[];
