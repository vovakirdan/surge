...
// continuation itself is unchanged: it keeps "compare has an unproved unmatched continuation". Over a union
// subject that continuation is the checker's to refuse (owner ruling 2026-09-25: only an unguarded,
// irrefutable arm covers its variant), so the continuation rows compare an `int` or a tuple, which the
// checker leaves unchecked.
...
// The unmatched continuation keeps its row, whether a guard, a constant or a partial tuple leaves it. The subjects are
// not unions: a union compare whose arms can all miss is refused by the checker before this analysis runs.
const originComparePatternRefusedSource = `fn guard_falls_through(n: int) -> int {
    return compare n {
        x if x > 5 => x;
        0 => 0;
    };
...
fn partial_tuple_only(t: (int, int)) -> int {
    return compare t {
        (0, y) => y;
        (x, 1) => x;
    };
...
func originComparePatternRefusedRows() []originPatternRow {
	...
	return []originPatternRow{
		...
		{name: "constants_keep_the_continuation_row", header: "fn constants_only(", body: "constants_only",
			want: []w{{"compare n {\n        1 => 1;\n        2 => 0;\n    }", originCompareContinuationRefusal}}, allow: originPatternDerived},
		{name: "partial_tuple_keeps_the_continuation_row", header: "fn partial_tuple_only(", body: "partial_tuple_only",
			want: []w{{"compare t {\n        (0, y) => y;\n        (x, 1) => x;\n    }", originCompareContinuationRefusal}}, allow: originPatternDerived},
