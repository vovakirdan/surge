...
func TestPreludeOptionAvailableInOpenFiles(t *testing.T) {
	dir := tempProjectDir(t)
	src := strings.Join([]string{
		"@entrypoint",
		"fn main() -> int {",
	...
	snapshot, diags, err := diagnose.AnalyzeFiles(context.Background(), &opts, []string{path}, diagnose.FileOverlay{})
	if err != nil {
