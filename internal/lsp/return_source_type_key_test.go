...
func TestReturnSourceLSPTypeKeys(t *testing.T) {
	for _, name := range []string{"legacy", "explicit", "nested", "empty"} {
		t.Run(name, func(t *testing.T) {
			...
			want := symbols.TypeKey("fn@return_source{0}(&string)->&string")
			switch name {
			case "legacy":
			...
			if got := typeKeyForType(in, id); got != want {
				t.Fatalf("LSP key = %q, want %q", got, want)
			}
		})
	}
}
