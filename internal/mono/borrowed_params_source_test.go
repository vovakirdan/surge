	...
	for _, tc := range []struct{ name, activation string }{{"host_async", "async"}, {"host_blocking", "blocking"}} {
		t.Run(tc.name, func(t *testing.T) {
			...
			walk := borrowedBodyWalker{expr: func(e *hir.Expr) error {
				...
				_, childOwners, childErr := PrepareBorrowedParamBody(child, in, []symbols.SymbolID{p})
				if childErr != nil || !slices.Equal(childOwners, []symbols.SymbolID{p}) {
					t.Fatalf("nested body did not actually write same symbol %d: %v, %v", p, childOwners, childErr)
				}
				children++
				return nil
			}}
			if walkErr := walk.block(fn.Body); walkErr != nil || children != 1 {
				t.Fatalf("activation witness: children=%d error=%v", children, walkErr)
			}
