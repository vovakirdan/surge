...
// private.
//
// The call is what the rows are about. An `int[]` shares no counted block, so
// the old gate emitted nothing for it at all -- no instruction in MIR, no body
...
fn probe(xs: int[], xss: int[][], ss: string[], h: IntHolder, p: PlainPair,
         t: (int, int), n: int, fs: float[], fx: Array<int>[4]) -> int {
    return n;
}
...
func TestUnshareWalkPassesNullForAnElementWithNothingToMakePrivate(t *testing.T) {
	labels := []string{
		"Array<int>", "Array<string>", "Array<float>", "Array<Array<int>>",
		"IntHolder", "ArrayFixed<Array<int>, const 4, 4>", "float",
	}
	...
	rows := []struct {
		...
		{"Array<int>", "i64 0", "8", "null", 1,
			"an int element has nothing to make private, so the runtime is asked only about the header"},
		...
		{"Array<Array<int>>", "i64 0", "8", "Array<int>", 1,
		...
		{"ArrayFixed<Array<int>, const 4, 4>", "i64 0", "8", "null", 4,
...
func TestUnshareWalkIsNotArmedForAValueWithNeitherAnArrayNorACount(t *testing.T) {
	_, ids, e := unshareProbeFrom(t, arrayWalkProbeProgram,
		"PlainPair", "(int, int)", "int", "Array<int>")
	for _, label := range []string{"PlainPair", "(int, int)", "int"} {
	...
	if !e.typeNeedsRelinquishWalk(ids["Array<int>"]) {
		t.Error("Array<int>: typeNeedsRelinquishWalk=false; the rows above would pass on a broken predicate")
	}
}

// The site, end of the emitter's side: a blocking capture of an `int[]` emits
// exactly one call of the array's walk body, before rt_blocking_submit hands
// the frame to the pool, and that body carries the null-callback buffer walk.
// At the base commit this program emitted NO call and NO body at all.
...
`)
	...
	if len(calls) != 1 || calls[0][3] != "null" {
		t.Fatalf("the int[] capture's walk is %v, want one buffer walk with a null callback:\n%s", calls, walk)
	}
}
