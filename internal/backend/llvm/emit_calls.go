...
func (fe *funcEmitter) emitCall(ins *mir.Instr) error {
	...
	if handled, err := fe.emitPlacementIntrinsic(call); handled {
		return err
	}
	if handled, err := fe.emitRuntimeIntrinsic(call); handled {
		return err
	}
	if handled, err := fe.emitAnchoredChannelSend(call); handled {
		return err
	}
	if handled, err := fe.emitChannelIntrinsic(call); handled {
		return err
	}
	if handled, err := fe.emitMapIntrinsic(call); handled {
		return err
	}
