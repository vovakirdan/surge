...
runtime-v2-crossing-check:
	...
	SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 $(GO) test ./internal/vm -run '^TestRuntimeV2(CancelledSuspendStateReclaimed|FarChannelNonCopyRoundTrip|FarSelectNonCopySendArm|FarSelectConstArmEvaluatedOnce|FarSelectCancelNonCopySendArm|FarHandleFieldDropReleasesTheLease|FarHandleHasOneOwnerAcrossACrossing|FarPayloadWidth|ResidentBytesTelemetry|ResidentBytesLedger|StructArrayElementsReclaimed|WhileBodyLocalReclaimed|AnchoredCancelInFlightKeepsOneHandleOwner|DebugQuiesceAnswers)$$' -count=1 -parallel=1 -p=1 --timeout 900s
