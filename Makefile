...
runtime-v2-crossing-check:
	...
	$(GO) test ./internal/backend/llvm -run '^Test(EmitCrossingChargesCopyCapturesOnce|EmitCrossingDoesNotChargeMovedCaptures|EmitCrossingsNameEveryPayloadType|EmitAnchoredStateGlueLeavesTheCallersHandleAlone|EmitAnchoredStateGlueNegativeControl|EmitFarSelectUnsharesTheSendPayloadOnce|EmitSpawnOnUnsharesTheResultBeforeAsyncReturn|EmitBlockingUnsharesTheResultBeforeItReturns|EmittedModuleWithAnUnshareAssembles|EmitDefinesNoUnshareBodyWithoutDemand|UnshareWalkOfABareCountedScalar|UnshareOfAContainerIsRefusedNamingSemasPredicate|UnsharePredicatesAgreeWithSema)$$' -count=1 --timeout 120s
	SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 $(GO) test ./internal/vm -run '^TestRuntimeV2(CancelledSuspendStateReclaimed|FarChannelNonCopyRoundTrip|FarSelectNonCopySendArm|FarSelectConstArmEvaluatedOnce|FarSelectCancelNonCopySendArm|FarHandleFieldDropReleasesTheLease|FarHandleHasOneOwnerAcrossACrossing|FarPayloadWidth|ResidentBytesTelemetry|ResidentBytesLedger|StructArrayElementsReclaimed|WhileBodyLocalReclaimed|AnchoredCancelInFlightKeepsOneHandleOwner|DebugQuiesceAnswers|Task9Matrix|Task9AnchoredStateReleaseNegativeControl)$$' -count=1 -parallel=1 -p=1 --timeout 1200s
