...
# ===== C Runtime Checks =====
# Every whole-tree C gate walks a roster discovered by `find`, and `find` on a
# missing directory is silenced at the C_SOURCES definition above. An empty
# roster therefore spells itself as a loop that runs zero times, and cfmt-check,
# c-warnings and ctidy each print their own OK line and exit 0 -- one rename of
# runtime/native turns the whole c-check-gcc CI job green while nothing at all is
# checked. cppcheck is the exception only by accident: its own empty-roster guard
# used to `exit 0` from a recipe line of its own, which ends that line's shell and
# leaves make walking on to the cppcheck call, where the TOOL refused an empty
# argument list. The gate never said no; the tool did.
#
# The preflight is a PREREQUISITE rather than a line inside each recipe so that a
# C gate added later cannot be written without it. The companion stand in
# internal/gatecheck holds exactly that shape.
#
# It runs BEFORE each gate's tool-availability check on purpose: an empty roster
# is the same answer with or without clang installed, and that is what lets the
# stand ask the question on a machine with no C toolchain at all.
#
# No count is pinned here. The roster grows, and a number in a Makefile rots the
# way a number in prose does; what has to hold is that it is not zero.
.PHONY: c-roster-preflight
c-roster-preflight:
	@if [ ! -d "$(C_RUNTIME_DIR)" ]; then \
		echo "Error: C_RUNTIME_DIR=$(C_RUNTIME_DIR) is not a directory, so the C gates would walk an EMPTY ROSTER and report OK without checking anything"; \
		exit 1; \
	fi
	@if [ -z "$(strip $(C_SOURCES))" ] || [ -z "$(strip $(C_HEADERS))" ]; then \
		echo "Error: EMPTY ROSTER under $(C_RUNTIME_DIR): $(words $(C_SOURCES)) sources, $(words $(C_HEADERS)) headers. A C gate with an empty roster passes without checking anything"; \
		exit 1; \
	fi
	@echo ">> C roster: $(words $(C_SOURCES)) sources, $(words $(C_HEADERS)) headers under $(C_RUNTIME_DIR)"

# Check C code formatting with clang-format
cfmt-check: c-roster-preflight
