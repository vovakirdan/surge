...
runtime-v2-abi-manifest-check:
	...
	SURGE_GATE_NAME=runtime-v2-abi-manifest-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeOpaqueABIFieldsAreFixed64,TestNetAsyncHandleSignaturesAreFixed64,TestTermEventResizeABIIsFixed64 -- $(GO) test ./internal/backend/llvm -run '^(TestRuntimeOpaqueABIFieldsAreFixed64|TestNetAsyncHandleSignaturesAreFixed64|TestTermEventResizeABIIsFixed64)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 120s
	...
	# The Range object is described twice, once per language, and neither
	# description can see the other: rt.h's _Static_asserts catch drift on the C
	# side only, because a C compiler cannot see a Go constant. These are the
	# other half -- the emitter's offsets and its bound-kind values against the
	# same numbers -- and they belong on this gate for the same reason the
	# typed-carrier rows do. A mismatch here is not a compile error but a
	# reference count taken four or eight bytes from where the count lives.
	$(GO) test ./internal/backend/llvm -run '^Test(RangeLayoutMatchesTheRuntimeHeader|ARangeConstructorNamesItsBoundKind|ARangeCursorRetainsTheBoundsItCopied|ARangeStepWalksItsBoundsWithTheirOwnArithmetic|TheRangeLiteralSpellingKeepsTheIntConstructors)$$' -count=1 --timeout 120s
...
runtime-v2-carrier-check:
	...
	SURGE_GATE_NAME=runtime-v2-carrier-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestEmitNumericCastTemporaryCleanup,TestEmitNumericCastBorrowedAndFixedControls,TestChannelSendOfferUsesDisposablePollStorage -- $(GO) test ./internal/backend/llvm -run '^(TestEmitNumericCastTemporaryCleanup|TestEmitNumericCastBorrowedAndFixedControls|TestChannelSendOfferUsesDisposablePollStorage)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 180s
	SURGE_GATE_NAME=runtime-v2-carrier-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestLowerUintLiteralPreservesKindAndText,TestChannelSendPollPreservesTheCountedCopyOwner -- $(GO) test ./internal/mir -run '^(TestLowerUintLiteralPreservesKindAndText|TestChannelSendPollPreservesTheCountedCopyOwner)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 120s
	SURGE_GATE_NAME=runtime-v2-carrier-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2EntrypointArgvBorrowsAndReleasesStorage -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^(TestRuntimeV2EntrypointArgvBorrowsAndReleasesStorage)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 120s
	SURGE_GATE_NAME=runtime-v2-carrier-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2AsyncAllocationXMLRejectsMalformedReports,TestRuntimeV2AsyncAllocationXMLExactMultiset,TestRuntimeV2AsyncAllocationBaselineOrigins -- $(GO) test ./internal/vm -run '^(TestRuntimeV2AsyncAllocationXMLRejectsMalformedReports|TestRuntimeV2AsyncAllocationXMLExactMultiset|TestRuntimeV2AsyncAllocationBaselineOrigins)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 120s
	...
	# Generated numeric bindings, latch exits and actual suspend-frame ownership.
	SURGE_GATE_NAME=runtime-v2-carrier-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestNumericForUsesOwnedCandidatesAndSinglePost,TestIteratorOwnershipCandidatesAndElementFallback,TestWhilePostNormalizationAndLegacyReleaseTraversal,TestWhilePostPrintingAndClassicForRemainDistinct -- $(GO) test ./internal/hir -run '^(TestNumericForUsesOwnedCandidatesAndSinglePost|TestIteratorOwnershipCandidatesAndElementFallback|TestWhilePostNormalizationAndLegacyReleaseTraversal|TestWhilePostPrintingAndClassicForRemainDistinct)$$' -count=1 -parallel=1 -p=1 -v --timeout 120s
	SURGE_GATE_NAME=runtime-v2-carrier-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestWhilePostCloneSubstitutionAndTypes,TestWhilePostSymbolRewritesAndDCEEdges,TestWhilePostTraversalErrorsAndNil,TestGeneratedLoopCandidateSurvivesMonomorphization -- $(GO) test ./internal/mono -run '^(TestWhilePostCloneSubstitutionAndTypes|TestWhilePostSymbolRewritesAndDCEEdges|TestWhilePostTraversalErrorsAndNil|TestGeneratedLoopCandidateSurvivesMonomorphization)$$' -count=1 -parallel=1 -p=1 -v --timeout 120s
	SURGE_GATE_NAME=runtime-v2-carrier-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestGeneratedLoopOwnershipUsesActualType,TestGeneratedResourceOnlyReplacesLegacyWholeLocalRelease,TestNumericPostActualOwnershipPreservesReferences,TestGeneratedLoopLexicalDropOrderAndInitializerFrontier,TestGeneratedLoopReturnDetachesValueBeforeDrops,TestGeneratedLoopBlockReturnsReleaseOnlyExitedFrames,TestNumericLoopLatchAndExitOwnership,TestNumericLoopPostDropsOnlyOwningConcreteLocals -- $(GO) test ./internal/mir -run '^(TestGeneratedLoopOwnershipUsesActualType|TestGeneratedResourceOnlyReplacesLegacyWholeLocalRelease|TestNumericPostActualOwnershipPreservesReferences|TestGeneratedLoopLexicalDropOrderAndInitializerFrontier|TestGeneratedLoopReturnDetachesValueBeforeDrops|TestGeneratedLoopBlockReturnsReleaseOnlyExitedFrames|TestNumericLoopLatchAndExitOwnership|TestNumericLoopPostDropsOnlyOwningConcreteLocals)$$' -count=1 -parallel=1 -p=1 -v --timeout 120s
	SURGE_GATE_NAME=runtime-v2-carrier-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestArrayIteratorFloatYieldRetainsOnce -- $(GO) test ./internal/backend/llvm -run '^(TestArrayIteratorFloatYieldRetainsOnce)$$' -count=1 -parallel=1 -p=1 -v --timeout 120s
	SURGE_GATE_NAME=runtime-v2-carrier-check SURGE_BACKEND=vm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestVMNumericIteratorFloatArrays,TestVMNumericIteratorFastLatch,TestVMNumericIteratorSuspendLifecycle,TestVMNumericIteratorHeapArrays,TestVMNumericIteratorHeapRanges,TestVMNumericIteratorHeapSuspendLifecycle -- $(GO) test ./internal/vm -run '^(TestVMNumericIteratorFloatArrays|TestVMNumericIteratorFastLatch|TestVMNumericIteratorSuspendLifecycle|TestVMNumericIteratorHeapArrays|TestVMNumericIteratorHeapRanges|TestVMNumericIteratorHeapSuspendLifecycle)$$' -count=1 -parallel=1 -p=1 -v --timeout 600s
	SURGE_GATE_NAME=runtime-v2-carrier-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestVMNumericIteratorFloatArrays,TestVMNumericIteratorFastLatch,TestVMNumericIteratorSuspendLifecycle,TestVMNumericIteratorHeapArrays,TestVMNumericIteratorHeapRanges,TestVMNumericIteratorHeapSuspendLifecycle -- $(GO) test ./internal/vm -run '^(TestVMNumericIteratorFloatArrays|TestVMNumericIteratorFastLatch|TestVMNumericIteratorSuspendLifecycle|TestVMNumericIteratorHeapArrays|TestVMNumericIteratorHeapRanges|TestVMNumericIteratorHeapSuspendLifecycle)$$' -count=1 -parallel=1 -p=1 -v --timeout 600s
	SURGE_GATE_NAME=runtime-v2-carrier-check SURGE_BACKEND=vm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestNumericIteratorSuspendFrameOwnsLiveBindings,TestNumericRangeInitializerOwnsTemporaryBoundsSource,TestNumericRangeInitializerFixedWidthSource -- $(GO) test ./internal/vm -run '^(TestNumericIteratorSuspendFrameOwnsLiveBindings|TestNumericRangeInitializerOwnsTemporaryBoundsSource|TestNumericRangeInitializerFixedWidthSource)$$' -count=1 -parallel=1 -p=1 -v --timeout 120s
...
runtime-v2-owned-storage-check:
	...
	SURGE_GATE_NAME=runtime-v2-owned-storage-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2BigfloatFromF64Exact,TestRuntimeV2BigfloatFromF64ExactUnderAddressAndUndefinedSanitizers,TestRuntimeV2RangeInlineBoundsAvoidNumericCalls,TestRuntimeV2RangeNumericCallsUnderAddressAndUndefinedSanitizers,TestRuntimeV2ChannelCloseThenCancelRetainsStagedSend,TestRuntimeV2ChannelCloseThenCancelUnderAddressAndUndefinedSanitizers,TestRuntimeV2ChannelCancelKeepsLastFrameHandle,TestRuntimeV2ChannelCancelFrameUnderAddressAndUndefinedSanitizers -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^(TestRuntimeV2BigfloatFromF64Exact|TestRuntimeV2BigfloatFromF64ExactUnderAddressAndUndefinedSanitizers|TestRuntimeV2RangeInlineBoundsAvoidNumericCalls|TestRuntimeV2RangeNumericCallsUnderAddressAndUndefinedSanitizers|TestRuntimeV2ChannelCloseThenCancelRetainsStagedSend|TestRuntimeV2ChannelCloseThenCancelUnderAddressAndUndefinedSanitizers|TestRuntimeV2ChannelCancelKeepsLastFrameHandle|TestRuntimeV2ChannelCancelFrameUnderAddressAndUndefinedSanitizers)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 1200s
	...
	SURGE_GATE_NAME=runtime-v2-owned-storage-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2ArrayUnshareWalkWalksAnOwnedBufferAndRefusesAView,TestRuntimeV2ArrayUnshareWalkUnderAddressAndUndefinedSanitizers,TestRuntimeV2ArrayUnshareWalkNegativeControlSkipsTheViewCheck -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^TestRuntimeV2ArrayUnshareWalk(WalksAnOwnedBufferAndRefusesAView|UnderAddressAndUndefinedSanitizers|NegativeControlSkipsTheViewCheck)$$' -count=1 -parallel=1 -p=1 -v --timeout 600s
	SURGE_GATE_NAME=runtime-v2-owned-storage-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2BignumRefcountRejectsAnUnknownRow,TestRuntimeV2BignumRefcountCountsHeapValuesAndPassesFixnumsThrough,TestRuntimeV2BignumRefcountUnderAddressAndUndefinedSanitizers,TestRuntimeV2BignumRefcountNegativeControlsCutEachBranch,TestRuntimeV2BiguintSubZeroAndOrder,TestRuntimeV2BiguintSubUnderAddressAndUndefinedSanitizers,TestRuntimeV2BiguintSubEqualNegativeControl -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^TestRuntimeV2(BignumRefcount(RejectsAnUnknownRow|CountsHeapValuesAndPassesFixnumsThrough|UnderAddressAndUndefinedSanitizers|NegativeControlsCutEachBranch)|BiguintSub(ZeroAndOrder|UnderAddressAndUndefinedSanitizers|EqualNegativeControl))$$' -count=1 -parallel=1 -p=1 -v --timeout 600s
	SURGE_GATE_NAME=runtime-v2-owned-storage-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRefCountedNumericScalarKindsAndQualifiers -- $(GO) test ./internal/types -run '^TestRefCountedNumericScalarKindsAndQualifiers$$' -count=1 -parallel=1 -p=1 -v --timeout 120s
	SURGE_GATE_NAME=runtime-v2-owned-storage-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestEmitNumericLifecycleHeapGuardsDominate,TestEmitNumericLifecyclePreservesFloatIR,TestEmitNumericLifecycleAliasesAndNonOwningControls,TestEmitNumericBoundsStepSkipsOnlyIntegerOneRelease,TestEmitNumericLifecycleNestedControlFlow -- $(GO) test ./internal/backend/llvm -run '^TestEmitNumeric(Lifecycle(HeapGuardsDominate|PreservesFloatIR|AliasesAndNonOwningControls|NestedControlFlow)|BoundsStepSkipsOnlyIntegerOneRelease)$$' -count=1 -parallel=1 -p=1 -v --timeout 180s
	SURGE_GATE_NAME=runtime-v2-owned-storage-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2NumericEmittedLifecycle,TestRuntimeV2NumericEmittedLifecycleUnderAddressAndUndefinedSanitizers -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^TestRuntimeV2NumericEmittedLifecycle(UnderAddressAndUndefinedSanitizers)?$$' -count=1 -parallel=1 -p=1 -v --timeout 600s
...
runtime-v2-carrier-sanitizer-check:
	...
	SURGE_GATE_NAME=runtime-v2-carrier-sanitizer-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2ChannelSendOfferTakeOrDrop,TestRuntimeV2ChannelSendOfferOldAPINegativeControl,TestRuntimeV2ChannelSendOfferRepeatedParkedCancellation,TestRuntimeV2ChannelSendOfferClaimKeepsSenderAlive -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^(TestRuntimeV2ChannelSendOfferTakeOrDrop|TestRuntimeV2ChannelSendOfferOldAPINegativeControl|TestRuntimeV2ChannelSendOfferRepeatedParkedCancellation|TestRuntimeV2ChannelSendOfferClaimKeepsSenderAlive)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 600s
	SURGE_GATE_NAME=runtime-v2-carrier-sanitizer-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2ChannelSendOfferValgrindBaseline,TestRuntimeV2AsyncAllocationBaselineRejectsRetainedChannel -- $(GO) test ./internal/vm -run '^(TestRuntimeV2ChannelSendOfferValgrindBaseline|TestRuntimeV2AsyncAllocationBaselineRejectsRetainedChannel)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 1200s
	SURGE_GATE_NAME=runtime-v2-carrier-sanitizer-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2NumericCastTemporariesValgrindZero,TestRuntimeV2UintLiteralValueValgrindZero,TestRuntimeV2BigfloatFromF64ExactValgrindZero,TestRuntimeV2EntrypointArgvStorageValgrindZero,TestRuntimeV2RangeNumericCallsValgrindZero,TestRuntimeV2ChannelCancelFrameValgrindBaseline,TestRuntimeV2FixnumFastPathHeapWitnessValgrindZero -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^(TestRuntimeV2NumericCastTemporariesValgrindZero|TestRuntimeV2UintLiteralValueValgrindZero|TestRuntimeV2BigfloatFromF64ExactValgrindZero|TestRuntimeV2EntrypointArgvStorageValgrindZero|TestRuntimeV2RangeNumericCallsValgrindZero|TestRuntimeV2ChannelCancelFrameValgrindBaseline|TestRuntimeV2FixnumFastPathHeapWitnessValgrindZero)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 1200s
	SURGE_GATE_NAME=runtime-v2-carrier-sanitizer-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2BigfloatFromF64Exact,TestRuntimeV2BigfloatFromF64ExactUnderAddressAndUndefinedSanitizers,TestRuntimeV2RangeInlineBoundsAvoidNumericCalls,TestRuntimeV2RangeNumericCallsUnderAddressAndUndefinedSanitizers,TestRuntimeV2ChannelCloseThenCancelRetainsStagedSend,TestRuntimeV2ChannelCloseThenCancelUnderAddressAndUndefinedSanitizers,TestRuntimeV2ChannelCancelKeepsLastFrameHandle,TestRuntimeV2ChannelCancelFrameUnderAddressAndUndefinedSanitizers -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^(TestRuntimeV2BigfloatFromF64Exact|TestRuntimeV2BigfloatFromF64ExactUnderAddressAndUndefinedSanitizers|TestRuntimeV2RangeInlineBoundsAvoidNumericCalls|TestRuntimeV2RangeNumericCallsUnderAddressAndUndefinedSanitizers|TestRuntimeV2ChannelCloseThenCancelRetainsStagedSend|TestRuntimeV2ChannelCloseThenCancelUnderAddressAndUndefinedSanitizers|TestRuntimeV2ChannelCancelKeepsLastFrameHandle|TestRuntimeV2ChannelCancelFrameUnderAddressAndUndefinedSanitizers)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 1200s
	SURGE_GATE_NAME=runtime-v2-carrier-sanitizer-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2NumericIteratorFloatArraysValgrindZero,TestRuntimeV2NumericIteratorFloatBoundsValgrindZero,TestRuntimeV2NumericIteratorSuspendValgrindBaseline,TestRuntimeV2RangeForFloatBoundsIterateAndAnswer,TestRuntimeV2NumericIteratorHeapArraysValgrindZero,TestRuntimeV2NumericIteratorHeapRangesValgrindZero,TestRuntimeV2NumericIteratorHeapSuspendValgrindBaseline -- $(GO) test ./internal/vm -run '^(TestRuntimeV2NumericIteratorFloatArraysValgrindZero|TestRuntimeV2NumericIteratorFloatBoundsValgrindZero|TestRuntimeV2NumericIteratorSuspendValgrindBaseline|TestRuntimeV2RangeForFloatBoundsIterateAndAnswer|TestRuntimeV2NumericIteratorHeapArraysValgrindZero|TestRuntimeV2NumericIteratorHeapRangesValgrindZero|TestRuntimeV2NumericIteratorHeapSuspendValgrindBaseline)$$' -count=1 -parallel=1 -p=1 -v --timeout 900s
	...
	SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2DropFarChannelHandleAndObjectValgrindZero,TestRuntimeV2CrossingStrictCensusValgrindZero,TestRuntimeV2MapOwnedEntriesValgrindZero,TestRuntimeV2BlockingCaptureValgrindZero,TestRuntimeV2BlockingCapturelessStateIsFreed,TestRuntimeV2BlockingBodyLocalIsReclaimed,TestRuntimeV2BlockingReadCaptureIsReclaimed,TestRuntimeV2BlockingConsumedCaptureIsReleasedOnce,TestRuntimeV2BlockingCopyCompositeCaptureIsReclaimed,TestRuntimeV2CancelledOwnedResultValgrindZero,TestRuntimeV2CancelledTaskReclaimsItsScope,TestRuntimeV2ChannelHandleValgrindZero,TestRuntimeV2MutexLockUnlockValgrindZero,TestRuntimeV2ColdTaskDropValgrindZero,TestRuntimeV2ColdTaskCancelledColdValgrindZero -- $(GO) test ./internal/vm -run '^TestRuntimeV2(DropFarChannelHandleAndObjectValgrindZero|CrossingStrictCensusValgrindZero|MapOwnedEntriesValgrindZero|BlockingCaptureValgrindZero|BlockingCapturelessStateIsFreed|BlockingBodyLocalIsReclaimed|BlockingReadCaptureIsReclaimed|BlockingConsumedCaptureIsReleasedOnce|BlockingCopyCompositeCaptureIsReclaimed|CancelledOwnedResultValgrindZero|CancelledTaskReclaimsItsScope|ChannelHandleValgrindZero|MutexLockUnlockValgrindZero|ColdTaskDropValgrindZero|ColdTaskCancelledColdValgrindZero)$$' -count=1 -parallel=1 -p=1 -v --timeout 2100s
	...
	SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2ReallocReleaseIsForgottenByTheViewRegistry,TestRuntimeV2ReallocReleaseUnderAddressAndUndefinedSanitizers -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^TestRuntimeV2ReallocRelease(IsForgottenByTheViewRegistry|UnderAddressAndUndefinedSanitizers)$$' -count=1 -parallel=1 -p=1 -v --timeout 600s
	SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2ArrayUnshareWalkWalksAnOwnedBufferAndRefusesAView,TestRuntimeV2ArrayUnshareWalkUnderAddressAndUndefinedSanitizers,TestRuntimeV2ArrayUnshareWalkNegativeControlSkipsTheViewCheck -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^TestRuntimeV2ArrayUnshareWalk(WalksAnOwnedBufferAndRefusesAView|UnderAddressAndUndefinedSanitizers|NegativeControlSkipsTheViewCheck)$$' -count=1 -parallel=1 -p=1 -v --timeout 600s
	SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2BignumRefcountRejectsAnUnknownRow,TestRuntimeV2BignumRefcountCountsHeapValuesAndPassesFixnumsThrough,TestRuntimeV2BignumRefcountUnderAddressAndUndefinedSanitizers,TestRuntimeV2BignumRefcountNegativeControlsCutEachBranch,TestRuntimeV2BiguintSubZeroAndOrder,TestRuntimeV2BiguintSubUnderAddressAndUndefinedSanitizers,TestRuntimeV2BiguintSubEqualNegativeControl,TestRuntimeV2BiguintSubValgrindZero -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^TestRuntimeV2(BignumRefcount(RejectsAnUnknownRow|CountsHeapValuesAndPassesFixnumsThrough|UnderAddressAndUndefinedSanitizers|NegativeControlsCutEachBranch)|BiguintSub(ZeroAndOrder|UnderAddressAndUndefinedSanitizers|EqualNegativeControl|ValgrindZero))$$' -count=1 -parallel=1 -p=1 -v --timeout 600s
	SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRefCountedNumericScalarKindsAndQualifiers -- $(GO) test ./internal/types -run '^TestRefCountedNumericScalarKindsAndQualifiers$$' -count=1 -parallel=1 -p=1 -v --timeout 120s
	SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestEmitNumericLifecycleHeapGuardsDominate,TestEmitNumericLifecyclePreservesFloatIR,TestEmitNumericLifecycleAliasesAndNonOwningControls,TestEmitNumericBoundsStepSkipsOnlyIntegerOneRelease,TestEmitNumericLifecycleNestedControlFlow -- $(GO) test ./internal/backend/llvm -run '^TestEmitNumeric(Lifecycle(HeapGuardsDominate|PreservesFloatIR|AliasesAndNonOwningControls|NestedControlFlow)|BoundsStepSkipsOnlyIntegerOneRelease)$$' -count=1 -parallel=1 -p=1 -v --timeout 180s
	SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2NumericEmittedLifecycle,TestRuntimeV2NumericEmittedLifecycleUnderAddressAndUndefinedSanitizers,TestRuntimeV2NumericEmittedLifecycleValgrindZero,TestRuntimeV2NumericEmittedLifecycleNegativeControls,TestRuntimeV2NumericCopyOwnershipValgrindZero -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^TestRuntimeV2Numeric(EmittedLifecycle(UnderAddressAndUndefinedSanitizers|ValgrindZero|NegativeControls)?|CopyOwnershipValgrindZero)$$' -count=1 -parallel=1 -p=1 -v --timeout 1200s
...
runtime-v2-crossing-check:
	...
	SURGE_GATE_NAME=runtime-v2-crossing-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestCountedBlockRefusalPaths,TestCountedBlockRefusalPlainAndReachable,TestCountedBlockRefusalIncomplete -- $(GO) test ./internal/sema -run '^(TestCountedBlockRefusalPaths|TestCountedBlockRefusalPlainAndReachable|TestCountedBlockRefusalIncomplete)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 120s
	SURGE_GATE_NAME=runtime-v2-crossing-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestCountedBlockDiagnosticRepairsAtAllSites,TestCountedBlockDiagnosticRecursiveRepairs,TestCountedBlockDiagnosticValidDeclarationsRepair,TestCountedBlockDiagnosticDoesNotOfferFalseRepair -- $(GO) test ./internal/buildpipeline -run '^(TestCountedBlockDiagnosticRepairsAtAllSites|TestCountedBlockDiagnosticRecursiveRepairs|TestCountedBlockDiagnosticValidDeclarationsRepair|TestCountedBlockDiagnosticDoesNotOfferFalseRepair)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 300s
	$(GO) test ./internal/crossinggate -count=1 --timeout 300s
	$(GO) test ./internal/buildpipeline ./internal/hir -run '^(TestCrossingBackendUnavailableMessages|TestCrossingBackendGuardsAreDefaultClosed|TestCrossingBackendGuardDoesNotMaskSemaErrors|TestCrossingBackendGuardsCoverImportedModules|TestVMAndUnknownBackendsKeepExecutableAsyncFormsGuarded|TestLLVMTransportCapabilityOpensAsyncSpawnOn|TestLLVMTransportCapabilityOpensAsyncImmediateOn|TestLLVMTransportCapabilityOpensAsyncFarTaskLifecycle|TestLowerOnCrossingBypassReturnsError|TestLowerSpawnOnCrossingBypassReturnsError|TestLowerFarTaskCrossingBypassReturnsError|TestLowerCrossingRepresentationWithExplicitCapability|TestRefCountedScalarCrossingsAreRefused|TestRefCountedScalarCapturesCross|TestFixedWidthFloatStillCrosses|TestSelectSendGivingOneBindingToTwoArmsIsRefused|TestSelectSendPayloadConsumedByAnotherAwaitIsRefused|TestSelectSendAcceptedShapesStillCompile|TestRefCountedScalarChannelElementsShip|TestRefCountedScalarChannelElementsStayRefused|TestAnchoredSendOfACountedScalarMustGiveTheBindingAway|TestRefCountedScalarResultsRideTheReply|TestRefCountedScalarResultsStayRefused|TestFloatArrayBlockingResultEmitsThroughTheBufferWalk|TestOptionalFloatArrayBuiltEmptyRidesABlockingResult|TestArrayBehindAHandleIsRefusedAtEveryCrossingGate|TestContainersWithoutAnArrayBehindThemStillCross|TestCrossingArrayViewCapturesAreRefused|TestCrossingArrayCapturesThatHoldNoViewStillCross|TestCrossingArrayCaptureIsReclaimedByTheBodyThatReadsIt|TestAnchoredSendOfACapturedArrayMustGiveTheBindingAway|TestAnchoredSendOfACapturedArrayIsAccepted|TestAnchoredSendOfAValueReadOutOfACapturedArrayIsNotRefused|TestRangeWithCountedBoundsCrosses|TestRangeBehindAHandleStaysRefused)$$' -count=1 --timeout 120s
	$(GO) test ./internal/mono ./internal/mir -run '^(TestMonoPreservesCrossingRepresentation|TestMIRCrossingRepresentationWithExplicitCapability|TestMIRCrossingValidationDefaultClosed|TestMIRAsyncCrossingSuspendRepresentation|TestRelinquishedOperandsMustBeUnshared|TestRelinquishedOperandShapesAfterTheSplit|TestUnshareInstructionPrints|TestUnshareOfAnAliasIsAnOwnershipFinding|TestFarSelectCountedSendPayloadIsAPrivateTemp|TestSpawnOnPollUnsharesTheResultBeforeSpent|TestBlockingReturnOfACountedValueIsUnshared|TestBlockingCaptureOfAnArrayBuiltEmptyPublishesItsElementUnion|TestRelinquishedArrayOperandsAreSubjectToBothRules|TestAnchoredSendOfAnArrayIsNotHeldToTheActRule)$$' -count=1 --timeout 60s
	$(GO) test ./internal/sema ./internal/driver ./internal/types -run '^(TestCrossingLowering.*|TestFunctionCrossingEffectInference|TestCrossingReadinessDebt024ModuleImportDoesNotRequireImportedEffects|TestAnchorLeaseMisuseIsRejected|TestMayShareCountedBlockWalksUnionPayloads|TestNeedsRelinquishWalkArmsEveryArrayCrossing|TestContainsDynamicArray|TestContainsDynamicArrayTerminatesOnACycleThroughAUnion|TestContainsDynamicArrayBehindHandleTerminatesOnACycleThroughAHandle|TestOnCrossingDiagnostics|TestShardMovableCaptureGateAgreesWithTheAxis|TestCapabilityShardMovableAxis|TestDynamicArrayCaptureRegistrationMatchesTheGatesVerdict)$$' -count=1 --timeout 60s
	$(GO) test ./internal/backend/llvm -run '^Test(EmitCrossingChargesCopyCapturesOnce|EmitCrossingDoesNotChargeMovedCaptures|EmitCrossingsNameEveryPayloadType|EmitAnchoredStateGlueLeavesTheCallersHandleAlone|EmitAnchoredStateGlueNegativeControl|EmitFarSelectUnsharesTheSendPayloadOnce|EmitSpawnOnUnsharesTheResultBeforeAsyncReturn|EmitBlockingUnsharesTheResultBeforeItReturns|EmitSpawnOnUnsharesTheCaptureBeforePublish|EmitImmediateOnUnsharesTheCopyCaptureBeforeExecute|EmitBlockingUnsharesTheCaptureBeforeSubmit|EmitAnchoredSendLeavesTheGivenCaptureUntouchedBeforeTheRing|EmittedModuleWithAnUnshareAssembles|EmitDefinesNoUnshareBodyWithoutDemand|UnshareWalkOfABareCountedScalar|UnshareOfAFloatArrayCallsTheBufferWalk|UnshareOfAMapIsRefusedNamingSemasPredicate|UnshareWalkHandsAContainersBufferToTheRuntime|UnshareWalkNestsThroughAnArrayOfArrays|UnshareWalkReadsAnElementUnionTheFunctionNeverBuilds|UnshareRefusesAHandleWhoseRingStaysBehind|UnsharePredicatesAgreeWithSema|UnshareWalkPassesNullForAnElementWithNothingToMakePrivate|UnshareWalkIsNotArmedForAValueWithNeitherAnArrayNorACount|EmitBlockingCaptureOfAnIntArrayCallsTheBufferWalk|EmitBlockingCaptureOfAFloatArrayStillPassesAnElementBody|EmitCrossingOfAPlainCompositeEmitsNoWalk|EmitFarSelectSendOfAnIntArrayWalksThePayloadOnce)$$' -count=1 --timeout 120s
	SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 $(GO) test ./internal/vm -run '^TestRuntimeV2(CancelledSuspendStateReclaimed|FarChannelNonCopyRoundTrip|FarSelectNonCopySendArm|FarSelectHeapLiteralIsReclaimed|FarSelectCancelNonCopySendArm|FarHandleFieldDropReleasesTheLease|FarHandleHasOneOwnerAcrossACrossing|FarPayloadWidth|ResidentBytesTelemetry|ResidentBytesLedger|UnshareClonesCountTheSharedCaptures|UnshareClonesAreZeroWithoutASibling|UnshareClonesNegativeControl|UnshareOfAResidentCapture|UnshareClonesOnTheWayIntoARemoteChannel|UnshareClonesOnTheWayIntoARemoteChannelNegativeControl|UnshareClonesOnTheWayBackAsAReply|UnshareClonesOnTheWayBackAsAReplyNegativeControl|StructArrayElementsReclaimed|WhileBodyLocalReclaimed|AnchoredCancelInFlightKeepsOneHandleOwner|DebugQuiesceAnswers|Task9Matrix|Task9AnchoredStateReleaseNegativeControl|UnshareWalksAFloatArrayBufferAtEveryCrossingSink|UnshareOfAFloatArrayClonesNothingWithoutASibling|UnshareOfAFloatArrayNegativeControl|UnshareWalksAnArrayOfArraysToItsFloatLeaves|UnshareWalksAnArrayOfPaddedCopyStructsToTheirFloatFields|UnshareWalksAnArrayFieldMovedBySpawnOn|UnshareOfAnArrayViewIsRefusedByTheRuntime|UnshareOfAnArrayViewNegativeControl|UnshareOfABaseWithALiveViewIsRefusedByTheRuntime|UnshareOfABaseWithALiveViewNegativeControl|UnshareOfABaseWalksAfterItsViewIsGone|UnshareOfAnIntArrayViewIsRefusedByTheRuntime|UnshareOfAnIntArrayViewNegativeControl|UnshareOfAnIntArrayViewInAnOnHolderIsRefused|UnshareOfAnIntArrayViewInAnOnHolderNegativeControl|UnshareOfAnIntArrayViewInASpawnOnHolderIsRefused|UnshareOfAnIntArrayViewInASpawnOnHolderNegativeControl|UnshareOfAnIntArrayViewPushedIntoAHolderIsRefused|UnshareOfAnIntArrayViewPushedIntoAHolderNegativeControl|UnshareOfAnIntArrayViewInAHolderSlotIsRefused|UnshareOfAnIntArrayViewInAHolderSlotNegativeControl|UnshareOfAnIntArrayViewInATupleIsRefused|UnshareOfAnIntArrayViewInATupleNegativeControl|UnshareOfAnIntArrayViewThroughAFarChannelIsRefused|UnshareOfAnIntArrayViewThroughAFarChannelNegativeControl|UnshareOfOwnedIntAndStringArraysIsNotRefused|UnshareOfAnIntArrayViewInAMapIsRefusedAtCompileTime|UnshareOfAnIntArrayViewInAChannelIsRefusedAtCompileTime|UnshareInt64MapWithoutAnArrayStillCrosses|UnshareInt64ChannelWithoutAnArrayStillCrosses|AnchoredSendGivesACapturedArrayAway|RangeWithCountedBoundsCrossesIntoBlocking|ArrayCursorCannotCrossIntoBlocking)$$' -count=1 -parallel=1 -p=1 --timeout 1800s
...
runtime-v2-h2-tripwire-check:
	...
	SURGE_GATE_NAME=runtime-v2-h2-tripwire-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestH2TripwireAwaitRunnerBuilds,TestH2TripwireDroppedCallBuilds,TestH2TripwireRefusedG5ModuleTimeout,TestH2TripwireImportAddsCoreRowsG5b,TestH2TripwireImportDifferentialRejectsTheTwin,TestH2TripwireTaskCheckRefusesLeaks,TestH2TripwireTaskCheckRefusesSpawnLeaks,TestH2TripwireSpawnRunnerBuilds,TestH2TripwireTaskCheckRefusesSelectLeaks,TestH2TripwireSelectRunnerBuilds,TestTaskCheckCrossingBodyIsAFrame,TestTaskCheckCrossingBodyKeepsSoundPrograms,TestH2TripwireTaskCheckRefusesSpawnOnLeaks,TestH2TripwireSpawnOnRunnerBuilds,TestTaskCheckSpawnOnBodyEdges,TestTaskCheckCrossingBodyPanicClosedFrame -- $(GO) test ./internal/driver -run '^(TestH2TripwireAwaitRunnerBuilds|TestH2TripwireDroppedCallBuilds|TestH2TripwireRefusedG5ModuleTimeout|TestH2TripwireImportAddsCoreRowsG5b|TestH2TripwireImportDifferentialRejectsTheTwin|TestH2TripwireTaskCheckRefusesLeaks|TestH2TripwireTaskCheckRefusesSpawnLeaks|TestH2TripwireSpawnRunnerBuilds|TestH2TripwireTaskCheckRefusesSelectLeaks|TestH2TripwireSelectRunnerBuilds|TestTaskCheckCrossingBodyIsAFrame|TestTaskCheckCrossingBodyKeepsSoundPrograms|TestH2TripwireTaskCheckRefusesSpawnOnLeaks|TestH2TripwireSpawnOnRunnerBuilds|TestTaskCheckSpawnOnBodyEdges|TestTaskCheckCrossingBodyPanicClosedFrame)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 300s
	SURGE_GATE_NAME=runtime-v2-h2-tripwire-check SURGE_BACKEND=vm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestH2TripwireNotRunnableG1OwnBinding,TestH2TripwireNotRunnableG1bOwnExpr,TestH2TripwireNotRunnableG3ScopeJoin,TestH2TripwireNotRunnableG4xAsyncEntry,TestH2TripwireNotRunnableRo6xAsyncEntry,TestH2TripwireNotRunnableRo7xAsyncEntry,TestH2TripwireWitnessControl,TestH2TripwireHarnessObservesExit,TestH2TripwireMatcherRejectsRecordedFaults,TestH2TripwireTaskCheckRefusesLeakedRuns,TestH2TripwireAwaitRunnerRuns,TestH2TripwireDroppedCallNeverRuns,TestRuntimeV2BorrowedLocalTaskJoinedInItsFrame,TestH2TripwireTaskCheckRefusesSpawnLeakedRuns,TestH2TripwireSpawnRunnerRuns,TestH2TripwireTaskCheckRefusesSelectLeakedRuns,TestH2TripwireSelectRunnerRuns,TestRuntimeV2CrossingBodyTaskOverItsCopy,TestH2TripwireTaskCheckRefusesSpawnOnLeakedRuns,TestH2TripwireSpawnOnRunnerRuns,TestH2TripwireSpawnOnBodySuspensionHangs,TestRuntimeV2SpawnOn -- $(GO) test ./internal/vm -run '^(TestH2TripwireNotRunnableG1OwnBinding|TestH2TripwireNotRunnableG1bOwnExpr|TestH2TripwireNotRunnableG3ScopeJoin|TestH2TripwireNotRunnableG4xAsyncEntry|TestH2TripwireNotRunnableRo6xAsyncEntry|TestH2TripwireNotRunnableRo7xAsyncEntry|TestH2TripwireWitnessControl|TestH2TripwireHarnessObservesExit|TestH2TripwireMatcherRejectsRecordedFaults|TestH2TripwireTaskCheckRefusesLeakedRuns|TestH2TripwireAwaitRunnerRuns|TestH2TripwireDroppedCallNeverRuns|TestRuntimeV2BorrowedLocalTaskJoinedInItsFrame|TestH2TripwireTaskCheckRefusesSpawnLeakedRuns|TestH2TripwireSpawnRunnerRuns|TestH2TripwireTaskCheckRefusesSelectLeakedRuns|TestH2TripwireSelectRunnerRuns|TestRuntimeV2CrossingBodyTaskOverItsCopy|TestH2TripwireTaskCheckRefusesSpawnOnLeakedRuns|TestH2TripwireSpawnOnRunnerRuns|TestH2TripwireSpawnOnBodySuspensionHangs|TestRuntimeV2SpawnOn)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 600s
	SURGE_GATE_NAME=runtime-v2-h2-tripwire-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestH2TripwireNotRunnableG1OwnBinding,TestH2TripwireNotRunnableG1bOwnExpr,TestH2TripwireNotRunnableG3ScopeJoin,TestH2TripwireNotRunnableG4xAsyncEntry,TestH2TripwireNotRunnableRo6xAsyncEntry,TestH2TripwireNotRunnableRo7xAsyncEntry,TestH2TripwireWitnessControl,TestH2TripwireHarnessObservesExit,TestH2TripwireMatcherRejectsRecordedFaults,TestH2TripwireTaskCheckRefusesLeakedRuns,TestH2TripwireAwaitRunnerRuns,TestH2TripwireDroppedCallNeverRuns,TestRuntimeV2BorrowedLocalTaskJoinedInItsFrame,TestH2TripwireTaskCheckRefusesSpawnLeakedRuns,TestH2TripwireSpawnRunnerRuns,TestH2TripwireTaskCheckRefusesSelectLeakedRuns,TestH2TripwireSelectRunnerRuns,TestRuntimeV2CrossingBodyTaskOverItsCopy,TestH2TripwireTaskCheckRefusesSpawnOnLeakedRuns,TestH2TripwireSpawnOnRunnerRuns,TestH2TripwireSpawnOnBodySuspensionHangs,TestRuntimeV2SpawnOn -- $(GO) test ./internal/vm -run '^(TestH2TripwireNotRunnableG1OwnBinding|TestH2TripwireNotRunnableG1bOwnExpr|TestH2TripwireNotRunnableG3ScopeJoin|TestH2TripwireNotRunnableG4xAsyncEntry|TestH2TripwireNotRunnableRo6xAsyncEntry|TestH2TripwireNotRunnableRo7xAsyncEntry|TestH2TripwireWitnessControl|TestH2TripwireHarnessObservesExit|TestH2TripwireMatcherRejectsRecordedFaults|TestH2TripwireTaskCheckRefusesLeakedRuns|TestH2TripwireAwaitRunnerRuns|TestH2TripwireDroppedCallNeverRuns|TestRuntimeV2BorrowedLocalTaskJoinedInItsFrame|TestH2TripwireTaskCheckRefusesSpawnLeakedRuns|TestH2TripwireSpawnRunnerRuns|TestH2TripwireTaskCheckRefusesSelectLeakedRuns|TestH2TripwireSelectRunnerRuns|TestRuntimeV2CrossingBodyTaskOverItsCopy|TestH2TripwireTaskCheckRefusesSpawnOnLeakedRuns|TestH2TripwireSpawnOnRunnerRuns|TestH2TripwireSpawnOnBodySuspensionHangs|TestRuntimeV2SpawnOn)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 900s

runtime-v2-panic-surface-check:
	$(GUARD) runtime-v2-panic-surface-check
	@echo ">> Running the panic-surface census gate"
	SURGE_GATE_NAME=runtime-v2-panic-surface-check SURGE_BACKEND=vm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestVMNumericCastFailureContract,TestRuntimeV2EntrypointArgvFailurePaths,TestDefaultOfARuntimeHandleIsTheNullHandle,TestAnUnmarkedLookalikeIsNotANullHandle,TestNullRuntimeHandleIsInertOnEveryLifetimePath,TestNullRuntimeHandleIsRefusedWithTheNativeWords,TestNullRangeIsRefusedByEveryVMReader,TestALiveRangeIsReadPastTheNullCheck -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^(TestVMNumericCastFailureContract|TestRuntimeV2EntrypointArgvFailurePaths|TestDefaultOfARuntimeHandleIsTheNullHandle|TestAnUnmarkedLookalikeIsNotANullHandle|TestNullRuntimeHandleIsInertOnEveryLifetimePath|TestNullRuntimeHandleIsRefusedWithTheNativeWords|TestNullRangeIsRefusedByEveryVMReader|TestALiveRangeIsReadPastTheNullCheck)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 600s
	SURGE_GATE_NAME=runtime-v2-panic-surface-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestVMNumericCastFailureContract,TestRuntimeV2EntrypointArgvFailurePaths,TestRuntimeV2DefaultRuntimeHandleIsNullOnBothBackends,TestRuntimeV2NullRangeIsRefusedByEveryNativeEntryPoint -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^(TestVMNumericCastFailureContract|TestRuntimeV2EntrypointArgvFailurePaths|TestRuntimeV2DefaultRuntimeHandleIsNullOnBothBackends|TestRuntimeV2NullRangeIsRefusedByEveryNativeEntryPoint)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 600s
...
runtime-v2-heap-check:
	...
	SURGE_GATE_NAME=runtime-v2-heap-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2FailfastReturnReleasesPreparedValue,TestRuntimeV2FailfastReturnReleasesPreparedComposite -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^(TestRuntimeV2FailfastReturnReleasesPreparedValue|TestRuntimeV2FailfastReturnReleasesPreparedComposite)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 1200s
	SURGE_GATE_NAME=runtime-v2-heap-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2TaskResultCensusBalanced,TestRuntimeV2AsyncPlainCompositeReturn -- $(GO) test ./internal/vm -run '^(TestRuntimeV2TaskResultCensusBalanced|TestRuntimeV2AsyncPlainCompositeReturn)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 1200s
	SURGE_GATE_NAME=runtime-v2-heap-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2NumericCastTemporariesValgrindZero,TestRuntimeV2UintLiteralValueValgrindZero,TestRuntimeV2BigfloatFromF64ExactValgrindZero,TestRuntimeV2EntrypointArgvStorageValgrindZero,TestRuntimeV2RangeNumericCallsValgrindZero,TestRuntimeV2ChannelCancelFrameValgrindBaseline,TestRuntimeV2FixnumFastPathHeapWitnessValgrindZero -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^(TestRuntimeV2NumericCastTemporariesValgrindZero|TestRuntimeV2UintLiteralValueValgrindZero|TestRuntimeV2BigfloatFromF64ExactValgrindZero|TestRuntimeV2EntrypointArgvStorageValgrindZero|TestRuntimeV2RangeNumericCallsValgrindZero|TestRuntimeV2ChannelCancelFrameValgrindBaseline|TestRuntimeV2FixnumFastPathHeapWitnessValgrindZero)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 1200s
	SURGE_GATE_NAME=runtime-v2-heap-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2AsyncAllocationBaselineRejectsRetainedChannel -- $(GO) test ./internal/vm -run '^(TestRuntimeV2AsyncAllocationBaselineRejectsRetainedChannel)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 600s
	SURGE_GATE_NAME=runtime-v2-heap-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2NumericIteratorFloatArraysValgrindZero,TestRuntimeV2NumericIteratorFloatBoundsValgrindZero,TestRuntimeV2NumericIteratorSuspendValgrindBaseline,TestRuntimeV2RangeForFloatBoundsIterateAndAnswer,TestRuntimeV2NumericIteratorHeapArraysValgrindZero,TestRuntimeV2NumericIteratorHeapRangesValgrindZero,TestRuntimeV2NumericIteratorHeapSuspendValgrindBaseline -- $(GO) test ./internal/vm -run '^(TestRuntimeV2NumericIteratorFloatArraysValgrindZero|TestRuntimeV2NumericIteratorFloatBoundsValgrindZero|TestRuntimeV2NumericIteratorSuspendValgrindBaseline|TestRuntimeV2RangeForFloatBoundsIterateAndAnswer|TestRuntimeV2NumericIteratorHeapArraysValgrindZero|TestRuntimeV2NumericIteratorHeapRangesValgrindZero|TestRuntimeV2NumericIteratorHeapSuspendValgrindBaseline)$$' -count=1 -parallel=1 -p=1 -v --timeout 900s
	...
	@echo ">> Running Runtime V2 VM channel lifetime strict-zero and converted-binding drop gate"
	SURGE_BACKEND=vm SURGE_SKIP_TIMEOUT_TESTS=0 $(GO) test ./internal/vm -run '^TestRuntimeV2VMChannelStrictZero$$|^TestRuntimeV2DropOfConvertedBindingDoesNotPanicOnVM$$' -count=1 -parallel=1 -p=1 -v --timeout 120s
	...
	#
	# The claim above was FALSE for two rows until 2026-09-04, and the way it
	# failed is the reason it is worth stating at all. Epic 22 Phase 1's own
	# gate -- the float reclamation strict-zero row its epic document calls the
	# gate -- was selected by no target here, skipped by `make check` (which
	# leaves SURGE_SKIP_TIMEOUT_TESTS at 1), and reached only by the nightly
	# tagged sweep, which runs `./internal/vm` wholesale with no -run and so
	# names nothing. A row nobody selects cannot go red for a reason anybody
	# reads. Both float rows are now on this line, which is also what makes the
	# sentence above true again.
	@echo ">> Running Runtime V2 strict-census valgrind gate"
	SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 $(GO) test ./internal/vm -run '^TestRuntimeV2CrossingStrictCensusValgrindZero$$|^TestRuntimeV2FloatReclamationValgrindZero$$|^TestRuntimeV2FloatUnionPayloadSurvivesItsConstructor$$|^TestRuntimeV2DropFarChannelHandleAndObjectValgrindZero$$|^TestRuntimeV2MapOwnedEntriesValgrindZero$$|^TestRuntimeV2CancelledOwnedResultValgrindZero$$|^TestRuntimeV2CancelledTaskReclaimsItsScope$$|^TestRuntimeV2BlockingCaptureValgrindZero$$|^TestRuntimeV2BlockingCapturelessStateIsFreed$$|^TestRuntimeV2BlockingBodyLocalIsReclaimed$$|^TestRuntimeV2BlockingReadCaptureIsReclaimed$$|^TestRuntimeV2BlockingConsumedCaptureIsReleasedOnce$$|^TestRuntimeV2BlockingCopyCompositeCaptureIsReclaimed$$|^TestRuntimeV2ChannelHandleValgrindZero$$|^TestRuntimeV2MutexLockUnlockValgrindZero$$|^TestRuntimeV2StringFromStrErrorTemporaryValgrindSafe$$|^TestRuntimeV2StringFromBytesErrorTemporaryValgrindZero$$|^TestRuntimeV2UnshareCapturesLeakNothing$$|^TestRuntimeV2UnshareIntoARemoteChannelLeaksNothing$$|^TestRuntimeV2UnshareReplyLeaksNothing$$|^TestRuntimeV2UnshareOfAFloatArrayLeaksNothing$$|^TestRuntimeV2UnshareOfAnIntArrayLeaksNothing$$|^TestRuntimeV2AnchoredSendOfACapturedArrayLeaksNothing$$|^TestRuntimeV2ComparePayloadIsFreedOnceByItsArm$$|^TestRuntimeV2RangeBoundLifecycleValgrindZero$$|^TestRuntimeV2BlockExprNormalExitReclaimsItsLocals$$' -count=1 -parallel=1 -p=1 -v --timeout 1800s
	...
	SURGE_GATE_NAME=runtime-v2-heap-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2NumericEmittedLifecycleValgrindZero,TestRuntimeV2NumericEmittedLifecycleNegativeControls,TestRuntimeV2NumericCopyOwnershipValgrindZero,TestRuntimeV2BorrowedParameterOwnershipValgrindZero,TestRuntimeV2BorrowedParameterOwnershipVM -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^(TestRuntimeV2Numeric(EmittedLifecycle(ValgrindZero|NegativeControls)|CopyOwnershipValgrindZero)|TestRuntimeV2BorrowedParameterOwnershipValgrindZero|TestRuntimeV2BorrowedParameterOwnershipVM)$$' -count=1 -parallel=1 -p=1 -v --timeout 1200s
	...
	SURGE_GATE_NAME=runtime-v2-heap-check SURGE_BACKEND=vm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2ChannelSendOfferPreservesOriginal -- $(GO) test ./internal/vm -run '^(TestRuntimeV2ChannelSendOfferPreservesOriginal)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 600s
	SURGE_GATE_NAME=runtime-v2-heap-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2ChannelSendOfferPreservesOriginal,TestRuntimeV2ChannelSendOfferValgrindBaseline -- $(GO) test ./internal/vm -run '^(TestRuntimeV2ChannelSendOfferPreservesOriginal|TestRuntimeV2ChannelSendOfferValgrindBaseline)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 1200s
	SURGE_GATE_NAME=runtime-v2-heap-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2ChannelSendOfferTakeOrDrop,TestRuntimeV2ChannelSendOfferOldAPINegativeControl,TestRuntimeV2ChannelSendOfferRepeatedParkedCancellation,TestRuntimeV2ChannelSendOfferClaimKeepsSenderAlive -- $(GO) test -tags runtime_v2_pending ./internal/vm -run '^(TestRuntimeV2ChannelSendOfferTakeOrDrop|TestRuntimeV2ChannelSendOfferOldAPINegativeControl|TestRuntimeV2ChannelSendOfferRepeatedParkedCancellation|TestRuntimeV2ChannelSendOfferClaimKeepsSenderAlive)$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 600s
...
runtime-v2-lifecycle-check:
	...
	@echo ">> Running Runtime V2 cold task rows (RV2-DEBT-370: created cold, published by spawn, await, cancel or join, discarded unrun)"
	SURGE_GATE_NAME=runtime-v2-lifecycle-check SURGE_BACKEND=llvm SURGE_SKIP_TIMEOUT_TESTS=0 bash scripts/runtime_v2_carrier_sanitizer_check.sh run --expect TestRuntimeV2ColdTaskDroppedNeverRuns,TestRuntimeV2ColdTaskSpawnedRuns,TestRuntimeV2ColdTaskAwaitedRuns,TestRuntimeV2ColdTaskCloneKeepsItAlive,TestRuntimeV2ColdTaskCancelPublishesIt,TestRuntimeV2ColdTaskDroppedMemberDoesNotHoldTheJoin,TestRuntimeV2ColdTaskJoinPublishesAnEscapedMember,TestRuntimeV2ColdTaskAffinePinHolds,TestRuntimeV2ColdTaskForeignLaneReachesTheScopeByEvent,TestRuntimeV2ColdTaskDiscardAndCancelRace,TestRuntimeV2ColdTaskConcurrentLastDrops,TestRuntimeV2ColdTaskJoinWalkRacesALastDrop,TestRuntimeV2ColdTaskUnderThreadSanitizer,TestRuntimeV2ColdTaskInlineClaimTakesOnlyTheLatest,TestRuntimeV2ColdTaskDropValgrindZero,TestRuntimeV2ColdTaskFailfastCancelsAColdMemberUnrun,TestRuntimeV2ColdTaskCancelledColdUnderThreadSanitizer,TestRuntimeV2ColdTaskCancelledColdValgrindZero -- $(GO) test ./internal/vm -run '^(TestRuntimeV2ColdTask(DroppedNeverRuns|SpawnedRuns|AwaitedRuns|CloneKeepsItAlive|CancelPublishesIt|DroppedMemberDoesNotHoldTheJoin|JoinPublishesAnEscapedMember|AffinePinHolds|ForeignLaneReachesTheScopeByEvent|DiscardAndCancelRace|ConcurrentLastDrops|JoinWalkRacesALastDrop|UnderThreadSanitizer|InlineClaimTakesOnlyTheLatest|DropValgrindZero|FailfastCancelsAColdMemberUnrun|CancelledColdUnderThreadSanitizer|CancelledColdValgrindZero))$$' -short=false -count=1 -parallel=1 -p=1 -v --timeout 1800s
