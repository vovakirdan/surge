...
**Этапы 1–3 закрыты. Этап 4 (Epic 22 Phase 2) в работе:** числовая поставка D1
закрыта 15.09.2026, вторая поставка D2 (`@return_source` и освобождение
локальных владельцев на обычном выходе из блока) — последняя работа эпика. Этап
и по исходному замыслу шёл после 23b.
...
| **4 — Epic 22 Phase 2** | **В работе: D1 (`int`/`uint` — счётные скаляры) закрыт 15.09.2026 на `codex/step7-d1-r2` (коммиты после `ab0a7395`), D2 (`@return_source` и освобождение локальных владельцев на обычном выходе из блока) в работе.** Закрыты RV2-DEBT-035/068/363, сужен 357, открыты 361 (стоп-кран за хендлом, шаг 8) и 362; остаток Sentrux D1 принят владельцем 15.09. Ниже — запись до D1: crossing barriers ЗАКРЫТЫ, Phase 2 не начата | Пункт 2 этого этапа был проверкой, вернул «нет» и превратился в работу; эта работа легла шагами 4, 5 и 6 (06.09–09.09.2026), у каждого свои концы: шаг 4 `66e09d66`…`342795c7`, шаг 5 `a7f2eac9`…`4d1749ca`, шаг 6 `06b61e24`…`671ae266` — одним диапазоном от `a7f2eac9` их не назвать, он начинается ПОСЛЕ закрытия шага 4 и оставляет его тринадцать коммитов снаружи. Buffer walk закрыт шагом 6. **Не начата именно Phase 2 — `int`/`uint` reclamation**, и она теперь пристраивается к ГОТОВОМУ механизму, а не наследует вопрос |
...
**Эта таблица — исходное состояние на момент написания плана; живую сводку даёт
таблица этапов вверху файла.** Строки ниже правились по мере закрытия, но фраза
про `d2956347` под таблицей относится к тому, давнему прогону и как описание
сегодняшнего дерева не читается.
...
| Epic 22 | Phases 0a/0b и локальный `float` готовы; **crossing barriers завершены (шаги 4, 5 и 6, 06.09–09.09.2026)** — отказ остаётся только для значения, чьи счётные блоки или динамический массив лежат ЗА ХЕНДЛОМ (таблица map, кольцо канала, слот результата задачи); `int`/`uint` не начаты | Осталась только Phase 2 (`int`/`uint`) |
...
2. ~~Подтвердить, что шесть float/composite crossing barriers из Wave E закрыты~~ **ПРОВЕРЕНО 04.09.2026: они НЕ
   ...
   **ПОПРАВКА 09.09.2026: барьеры ПОСТРОЕНЫ, и все четыре факта, на которых держался абзац выше, сегодня
   ложны.** Абзац оставлен как есть — он датированная запись того, что дерево говорило 04.09, и объясняет,
   почему проверка превратилась в работу. Работа легла шагами 4, 5 и 6 (06.09–09.09.2026), последний из них —
   buffer walk. Что дерево говорит теперь, перепроверено по файлам, а не по коммит-сообщениям:
   (а) `internal/valueops/flags.go:226-234` даёт обоим слотам `cross_move_init` и `cross_clone_init`
   `fill: filledByBackendDerivedBody` (посадки `9d6aba75` и `632a7d5e`), так что `filledNowhere` про них больше
   не читается; (б) у `rt_bigfloat_clone` есть вызывающий — `internal/backend/llvm/emit_cross_glue.go:399`
   эмитирует `call ptr @rt_bigfloat_clone(...)`; (в) фразу «crossing has no descriptor support yet» сам
   рантайм уже отозвал: `runtime/native/rt_value_ops.c:265-267` пишет, что typed-carrier cutover (Epic 23b)
   сделал её ложной и она исправлена 04.09.2026; (г) `TestRefCountedScalarCrossingsAreRefused`
   (`internal/buildpipeline/crossing_refcounted_scalar_test.go`) менялся последний раз в `492f001b`, а до того
   в `520f8835` и `4d1749ca` — «не менялся с `3e9eb684`» неверно. Единственное, что из абзаца устояло:
   `plan_cross` по-прежнему никем не диспетчеризуется в рантайме (`rt_slot_control.c:42` — это проверка на
   NULL, а не вызов). Решение владельца, которого ждал последний абзац, принято 04.09.2026 — вариант (2), и он
   исполнен: осталась только Phase 2 (`int`/`uint`), и она пристраивается к готовому механизму.
3. Ownership:
   ...
   **Уточнение владельца 14.09.2026: обычный выход блока и ссылочный результат.**
   Исправление освобождения локального `Map` принимается вместе с доказательством,
   что заём уничтожаемого владельца не выходит через результат или внешнюю
   переменную. Согласован контракт `@return_source`: вывод по доступному телу,
   проверяемые пометки параметров, default всех ссылочных входов для opaque
   callable, явные параметры/`self` как источники в первой версии. Нормативное
   описание — в `RUNTIME_V2.md`, объяснение — в `RUNTIME_MODEL_EXPLAINED.ru.md`.
   Приёмка включает сохранение контракта через импорты, callback values и generic
   подстановки; анализ ветвей, циклов и рекурсивных вызовов; публичную диагностику
   до HIR; неизменные отрицательные проверки исключительного доступа; одинаковые
   фикстуры до/после и физическую проверку освобождения. Успех этой части сам по
   себе не закрывает исходный Map-бенчмарк, требования качества и весь Step 7.
...
5. Performance (протокол обновлён 04.09.2026 — ниже была записана редакция, которую решения того же дня заменили):
    ...
    - протокол замера — тот, что принял владелец 04.09.2026 и под который уже написан харнес
      ...
      сторона меряется на 24 физических копиях, каждая получает один warmup и пять парных замеров;
      в счёт идёт медианная по throughput копия среди прошедших CV ≤5% (нижняя медиана при чётном числе).
      Батчи ×8, p95 сравнивается только при p95 ≥1000 нс с обеих сторон; CV p95 проверяется отдельно для
      каждой стороны, достигшей этого порога. Ни выбросы, ни неудачные повторы вручную не удаляются. Прежняя редакция
      («2 warmup + 7 alternating paired runs, CV каждой стороны ≤5%») описывала протокол до этих решений;
    - throughput ≥95%; если медианные батчи обеих сторон короче 1 мс — ≥90%; p95 ≤110% там, где гейт применяется; zero
    ...
    - Уточнение владельца 11.09.2026: закрытая 23b база — `ea50ca0b6f22164bd2c615d73aff5d31670f2611`;
      непосредственная база изменений Step 7 — `ab0a73950402b471f25c63bffa87747650d04da6`.
      Это разные сравнения. Обновлённый протокол получает собственный SHA; он не выдаётся за исходный `ab0a7395`.
      Выделенный Ryzen теперь имеет 32 logical CPU и kernel `6.8.0-139-generic`; fixture CPU — `8,10`,
      harness — `0,2`. Замеры требуют отсутствия конкурирующих задач и очереди CI на всём сервере.
