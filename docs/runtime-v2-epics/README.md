...
| 25 | `25-ownership-verifier-and-tooling.md` | **Proposed, in progress.** An MIR-level ownership verifier and supporting investigation tooling, motivated by five ownership rows (`RV2-DEBT-097/098/099/100`, `052`'s residual) all closed the slow way this session — hand-built reproducer, `valgrind`, an eyeballed MIR dump. NOT part of the 22→23→24 detour chain and does not block Epic 23 Phase 2. The design went through four rounds of scoped `codex` review before settling; see the doc's own "Codex In The Loop" section and its `Traps` for what each round found. |
