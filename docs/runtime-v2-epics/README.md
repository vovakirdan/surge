...
| 22 | `22-numeric-reclamation.md` | **PARTIAL — active. Phase 2's numeric delivery (D1: `int`/`uint` as counted scalars) landed 2026-09-15 on `codex/step7-d1-r2` — RV2-DEBT-035, 068 and 363 closed, 357 narrowed, 361 (the counted stop-gap behind a handle, Step 8) and 362 opened; the second delivery (D2: `@return_source` and the normal block-exit release of local owners) is in flight and is all that is left.** The crossing barriers closed as steps 4, 5 and 6 (landed 2026-09-06/09). Phases 0a/0b/1 shipped: the ownership axes were split out of `IsCopy`, and `float` became a reference-counted scalar with a strict-zero valgrind gate. Every crossing un-shares its relinquishing operand, so a `float` or a `@copy` composite carrying one crosses as a capture, a channel element and a reply; and since step 6 a DYNAMIC ARRAY's buffer is walked element by element by the runtime before it leaves its shard, so a `float[]` rides an `own` capture of `on`, `spawn on` or `blocking`, a `blocking` result and a remote channel — but never an `on`/`spawn on` REPLY, which takes plain-copy data only and refuses an array with `FUT7020` — while a VIEW — or a base a live view still reads — is refused by name (`rt_array_unshare_walk`). What stays refused is no longer the buffer walk but a value whose counted blocks OR whose dynamic array sit BEHIND A HANDLE: a map's table, a channel's ring, a task's result slot (`CountedBlockStaysShared` and `DynamicArrayStaysUnchecked`; RV2-DEBT-038 carries the record). The history below is what the tree said on 2026-09-04, kept as written — **and every present-tense claim in it is now FALSE except one, so read it as a dated snapshot and not as the tree**: both cross slots are `filledByBackendDerivedBody` (`internal/valueops/flags.go:226-234`), `rt_bigfloat_clone` has a caller (`internal/backend/llvm/emit_cross_glue.go:399`), `rt_value_ops.c:265-267` has itself withdrawn the "no descriptor support yet" sentence, and `TestRefCountedScalarCrossingsAreRefused` last changed at `492f001b`. The one claim that survives is that no caller dispatches `plan_cross` anywhere in the runtime. the six barriers were NOT closed then — none of them. This line said Epic 23b's generic cross operations had absorbed them; the tree refutes that, and the epic's own header (`22-numeric-reclamation.md:1-12`) was right all along. `internal/valueops/flags.go` leaves `cross_move_init` and `cross_clone_init` `filledNowhere`, so `FlagShardMovable`/`FlagCrossClonable` are refused outright and `plan_cross` binds the trap for every descriptor the registry can hold; `runtime/native/rt_value_ops.c` states "crossing has no descriptor support yet" and leaves both cross slots NULL; and `rt_bigfloat_clone` is still declared, registered and never called. A crossing carrying an arbitrary-precision scalar is REFUSED, not copied, and `TestRefCountedScalarCrossingsAreRefused` has pinned that unchanged since `3e9eb684` (2026-07-24). Epic 22 resumes with Phase 2 `int`/`uint` reclamation, whose scope depends on how those barriers are answered: `int` cannot take the refusal `float` takes, because the stdlib `@copy` structs and the crossing fixtures are built on it. **ANSWERED 2026-09-04, variant (2): build the barriers for all three types first, then add `int`/`uint` to a finished mechanism** — which is this epic returning to its own Owner Decision 1, that already recorded the barriers as Phase 1 work. Two corrections came with the answer and are recorded in the epic: the "six paths" map was written against the pre-23b word ABI and is re-derived (three shapes over four MIR paths, six runtime state-entry points, four gates), and the stdlib blast radius is two modules and six types rather than the five modules the prose named. A second ruling fixed what the capability verdicts mean: `ShardMovable(float)` stays true because an exclusive `own` move hands over one obligation rather than sharing a counted block, `CrossClonable` means "possible via deep clone" rather than "raw bits copy", and flag, descriptor, registry hash and `Dump` derive from one backed state — a late mask in `backedFlags` is refused. |
...
| 23b | `23-storage-model-and-typed-carrier-abi.md`; `23b-inline-storage-and-typed-carriers.md` | **COMPLETE 2026-09-04.** Phase 2 end to end, waves A through F: inline value storage, destination-oriented calls, typed arrays/maps/tasks/channels/select/blocking/far transport, result-entitlement cloning, diagnostics, and deletion of the old erased ABI. The census reads 0 in every category a wave owned. The generic `float`/composite cross-clone barriers were IN SCOPE here and DID NOT ship, which this row used to claim they had. That point stands; the present-tense evidence it was written with does not, and is corrected 2026-09-09 rather than deleted. It said both `cross_move_init` and `cross_clone_init` were `filledNowhere` in `internal/valueops/flags.go` and NULL in the runtime's only hand-written descriptor, so no cross operation was dispatchable and Epic 22's barriers stayed open. Today both slots are `filledByBackendDerivedBody` (`flags.go:226-234`), and the barriers do NOT stay open — they landed under Epic 22's name as steps 4, 5 and 6 (2026-09-06/09). What remains true is the historical fact this row exists to record: 23b did not deliver them. Physical BYTE credits are not part of what shipped and are not deferred: the 2026-08-29 ruling found pointer transport charges no per-message bytes -- the message carries a pointer into a refcount graph the transport neither copies nor owns -- so the budget is SLOTS, and the admission park (RV2-DEBT-031) is what answers saturation. |
...
Epic 23b would carry. It did not carry them, so they became Epic 22's — and Epic
22 BUILT them, as steps 4, 5 and 6 across 2026-09-06/09. **So Phase 2 now
inherits a MECHANISM, not a question**, which is exactly what the owner's
variant (2) ruling of 2026-09-04 was chosen to produce; the sentence that
followed here said the opposite and is corrected 2026-09-09. The scope reasoning
it was making is kept, because it is why `int` was never allowed to take the
refusal `float` could, and it still governs what Phase 2 must do. It matters for
scope rather than for order: `float` can stay refused at a boundary because a program
