...
| 7 | `07-remote-select-symmetry.md` | In Progress | recon done (commit already atomic + self-reporting via winner index; far channels Copy-only by construction — sema gap is defense-in-depth); scope split: sema symmetry + race/idempotency/refusal rows NOW, abandon-time commit-bit reconciliation + non-copy e2e DEFERRED to vertical 3 with recorded design (d2) | 3, 5 |
