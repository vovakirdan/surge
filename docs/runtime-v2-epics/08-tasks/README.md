...
| 12 | `12-performance-benchmark-and-ci-gate.md` | Complete (post-F2 re-baseline recorded as the epic's performance record; per-commit trace-counter gate `TestRuntimeV2PerfControlLaneGate` wired via `runtime-v2-perf-check`→`runtime-v2-check`; 8x1024 control 12.86/req total, 9.36/req steady-state << 26.4 baseline; 8-shard ≥ 1-shard; RV2-DEBT-016 CLOSED) | trace/benchmark/CI | 10, 11 |
| 13 | `13-large-file-and-quality-tranche.md` | Complete (task park/wake cluster extracted verbatim to `rt_task_park.c`; `rt_async_state.c` 1386→1184 eff LOC, ceiling 1580→1184, RV2-DEBT-003 held open with 3 named remaining split candidates; stale executor-invariant comment rewritten to the 3-lane model; accept-static-test park_current pin split; behavior identical, full runtime-v2-check green incl. CompletionPinInterleavingTSan 1/2/8; sentrux all-rules-pass, −41 code-scope coupling drop accepted per RULES.md G3) | refactor | 10, 11, 12 |
| 14 | `14-epic-closeout.md` | Complete (contract sweep proves both contracts clause-by-clause; RV2-DEBT-015/016/019 closed, RV2-DEBT-021 closed via new deterministic cross-owner scope test; RV2-DEBT-020 comment corrected + owner reassigned to net-handle/accept epic (T8-N2 subsumed); RV2-DEBT-023 raised (cancel RUNNING→WAITING lost-cancellation, found in the T8-supplemental re-derivation); RV2-DEBT-022 carried; T8-N1 doc wording fixed; NOTES consolidated + RUNTIME_V2 flowback; mandatory syntax/Phase-4 warning recorded) | closeout | all |
