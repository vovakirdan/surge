...
**Что сегодня.** Правило по факту такое: **побеждает первая регистрация**. Задача,
созданная внутри асинхронного тела, регистрируется там же — то есть членство
действительно решается при создании. Но задача, созданная **вне любого**
асинхронного тела, не регистрируется нигде, и тогда её запуск внутри области даёт
ей **половину членства**: идентичность области проставляется, а в счётчике детей
её нет. Такая задача не ждётся `join_all` и не может быть отменена своей
областью — **но может поднять её fail-fast**. Это худший из вариантов: не член, но
влияет.

```surge
// ✅ компилируется сегодня
let t = async { ret slow().await(); };      // создана ВНЕ блока
let outcome = (@failfast async {
    let started = spawn t;                  // запущена ВНУТРИ блока
    let _ = started.await();
    ret 0;
}).await();
```

**Что предлагается.** Достроить правило до конца: членство фиксируется там, где
задача **создана**, включая случай «создана вне любой области» — тогда она просто
не член, и половинчатого состояния не возникает.

Важное ограничение, которое уже записано в модели: **идентичность области у
задачи пишется ровно один раз**, второй писатель — нарушение контракта. Поэтому
усыновление не может быть «перепиской» живой задачи. Если оно нужно, оно обязано
потреблять **холодную** задачу (ещё не запущенную) и отвергать ту, что уже
зарегистрирована где-то ещё:

```surge
// 🅿️ предлагается: явное вступление в бригаду
let t = async { ret slow().await(); };
let outcome = (@failfast async {
    let started = @adopt spawn t;
    let _ = started.await();
    ret 0;
}).await();
```

И диагностика на старую молчаливую форму:

```
error: `spawn` of a task created outside this scope does not make it a member
  note: the task was created here, outside `@failfast`
  help: create it inside the scope, or write `@adopt spawn t` to say it joins
```

Три предложения — `@local`, `@detached`, `@adopt` — живут в одном списке
атрибутов spawn-выражения, и новых ключевых слов язык не получает.
