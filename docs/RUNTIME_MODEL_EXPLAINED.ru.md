...
### Источник возвращаемой ссылки

Решение владельца от 14.09.2026 уточняет обычный выход из блока: локальные
владельцы освобождают свои значения, а ссылка на эти значения не может остаться
снаружи. Это касается и результата блока, и ссылки, записанной во внешнюю
переменную. Если блок возвращает само владеющее значение, его обязанность
освобождения переходит получателю.

Когда тело функции известно, компилятор сам выводит возможные источники
возвращаемой ссылки. Для интерфейса функции источник можно указать явно:
...
Здесь функция обещает, что ссылка в результате происходит из `a`. Передача
короткоживущего значения через `b` сама по себе не сокращает жизнь результата.
Компилятор проверяет обещание по телу функции. Несколько пометок разрешают
несколько возможных источников. У неизвестной функции без пометок источником
может оказаться любой ссылочный вход, включая `self`.

В первой версии источники выражаются через параметры: для возврата ссылки из
захваченной переменной её нужно передать явно. Отсутствие доказательства
происхождения не делает ссылку безопасной. Эти правила не продлевают жизнь
владельца и не дают дополнительных прав на изменение значения; их приёмка
входит в план завершения Epic 22 Phase 2.
...
// ✅ исполняется на LLVM: тело работает на другом шарде со своими копиями
//    захватов, результат приходит через `far Task<T>.await()`; VM `spawn on`
//    не исполняет (FUT7015). Тело, которое ждёт (`.await()`) на другом шарде,
//    сегодня зависает (RV2-DEBT-344)
let remote = spawn on distributed { ret compute(own input_b); };

// ⚠️ компилируется, но при исполнении падает детерминированно: размещение
//    `pool` бэкенд пока не поддерживает ("spawn on placement is not supported")
...
| Пересечения границ | синтаксис есть (`spawn on …`, `far Task<T>`); на LLVM `distributed` и `shard(n)` исполняются, `pool` и VM — нет |
...
| Значение по умолчанию у хендла рантайма (`Task`, `Channel`, `Range`) | нулевой хендл: сброс и копирование ничего не делают, любая операция над ним — ошибка времени выполнения, одинаковая на обоих бэкендах (`invalid task handle`, `async: null channel handle`, `null range handle`); нулевой `Range` — не «весь диапазон» |
