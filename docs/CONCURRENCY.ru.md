...
> **Область:** async/await, Task/TaskResult, task, каналы, отмена, таймауты
...
- `task` **планирует** задачу для конкурентного выполнения.
...
- `async fn` возвращает `Task<T>` немедленно; она не запускается, пока не будет вызвана `await` или `task`.
...
- `task` планирует задачу и возвращает дескриптор `Task<T>`.
- Только `own` значения могут пересекать границу task.
- Типы с `@nosend` отвергаются в task (`SemaNosendInSpawn`).
- `task checkpoint()` вызывает предупреждение как бесполезный вызов (`SemaSpawnCheckpointUseless`).
- `spawn` зарезервирован для routines/parallel runtime; используйте `task` для async задач.
...
- **F2 (один poll за шаг):** каждый шаг планировщика выполняет ровно один poll;
  ...
  переочередляется.
- **F3 (детерминизм):** в обычном режиме порядок FIFO по task/wake; в fuzz-режиме
