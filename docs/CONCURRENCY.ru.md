...
- Ожидание в контексте задач реализовано через **park/unpark**; worker продолжает выполнять другие задачи.
...
- Фактические бэкенды: native/LLVM — MT-исполнитель; VM — одно-worker и ориентирован на корректность/диагностику.

Это сохраняет модель владения корректной между worker-потоками без межпоточной проверки заимствований.
...
Все ожидания в контексте задач (каналы, таймеры, join, select, готовность сети)
реализованы через park/unpark в рантайме.
...
- Таймеры VM по умолчанию работают в виртуальном времени. Реальное время
  доступно только в VM (`surge run --backend=vm --real-time`).
- Таймеры native/LLVM используют время исполнителя (виртуальное) и сейчас не
  имеют переключателя на реальное время.
...
- Бэкенды native/LLVM используют MT-исполнитель; VM однопоточный.
...
- Количество worker'ов задается рантаймом (например, `SURGE_THREADS`); значение
  по умолчанию основано на числе CPU.

### 10.1.1 VM бэкенд vs native/LLVM

- VM существует ради корректности, диагностики и детерминированного исполнения в разработке.
- Планирование VM однопоточное и может использовать виртуальное время или fuzz-режим; native/LLVM — MT и используют реальное время таймеров.
- Часть возможностей бэкенд-специфична: `blocking { ... }` поддерживается только в native/LLVM, VM его отклоняет.
...
### 10.2.1 Ожидание каналов и sync wrappers

Прямые операции каналов в телах `async fn` являются точками приостановки:
native runtime паркует задачу и оставляет executor workers доступными.

Операции каналов внутри синхронных helper-функций используют sync channel
fallback. Если async-код вызывает такой helper, рантайм не может приостановить
sync stack helper'а; он может временно припарковать executor worker и положиться
на compensation fallback для progress. Держите горячие async request/reply paths
в async-коде или добавляйте async-вариант helper'а для таких путей.

Используйте `blocking { ... }` только для OS-блокирующей работы. Это не способ
сделать горячий channel request/reply дешевле.
...
- **VM mode:** однопоточный детерминированный FIFO по умолчанию; опциональный
  fuzz-режим с фиксированным seed для воспроизводимых интерливингов (только VM).

### 10.4 Политика тестирования (parity vs MT)

- VM/LLVM parity tests принудительно используют `threads=1`, чтобы сравнивать
  backend semantics, а не scheduler interleavings.
- MT correctness проверяется отдельным MT test suite, который запускается с
  workers > 1.

### 10.5 Справедливость и CPU-bound работа

Справедливость гарантируется для задач **Ready** **детерминированным
одно-worker профилем планировщика VM** при кооперативном планировании. Область
действия важна: F1-F3 ниже — свойства именно этого профиля. Это не свойства
абстракции `checkpoint()` и не свойства одно-worker исполнения вообще —
нативный рантайм на одном worker'е их не наследует:
...
В нативном рантайме `checkpoint().await()` завершает текущий poll, проверяет
отмену и возвращает задачу в scheduler selection. Он **не** обещает, что перед
следующим poll этой задачи будет выполнена другая Ready-задача, ни при каком
числе worker'ов. Кооперация через `checkpoint()` даёт планировщику возможность
выполнить другую работу; возможность не равна гарантии очередности. Программы,
которым нужен конкретный порядок, обязаны выразить его каналом, join'ом или
другой явной зависимостью, а не полагаться на форму интерливинга. Решение
владельца модели от 2026-08-31; см. `docs/RUNTIME_V2.md`, «What `checkpoint()`
Promises».
...
- Поддержка бэкендов: только native/LLVM. VM отклоняет `blocking { ... }`,
  потому что он однопоточный и не имеет blocking pool.
- Размер blocking pool задается рантаймом (по умолчанию равен числу worker'ов;
  можно переопределить через `SURGE_BLOCKING_THREADS`).
...
## 12. Будущий строгий nonblocking режим (`pragma strict::nonblocking`)

`pragma strict::nonblocking` **зарезервирован** и пока не имеет эффекта.

Текущее поведение:

- `@nonblocking` проверяется для функций (см. `docs/ATTRIBUTES.md`).
- Компилятор отклоняет вызовы известных операций "может ожидать" из
  `@nonblocking` функций (например, `Mutex.lock`, `Condition.wait`,
  `Semaphore.acquire`, `Channel.send`/`recv`/`close`).
- `@waits_on("field")` проверяется и конфликтует с `@nonblocking`.

Если/когда `strict::nonblocking` будет реализован, он применит те же проверки
транзитивно к коду в task-контексте. Это только будущая работа.
...
- `blocking { ... }` не поддерживается в VM.
...
- `@nonblocking` сейчас считает операции каналов (`send`/`recv`/`close`) блокирующими,
  из-за чего в `@nonblocking` коде могут запрещаться распространенные async
  паттерны; если `strict::nonblocking` появится, это различие потребует более
  явных рекомендаций.
