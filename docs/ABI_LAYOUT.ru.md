...
- `owner` — невладеющая обратная ссылка на хранилище исходной строки; view её не удерживает и не освобождает.
- `ptr` указывает на непрерывные байты UTF-8 этой строки.
- `len` — длина исходной строки в байтах на момент взятия view.
- View заимствует свою строку: строка должна пережить каждое использование view. Компилятор проверяет это как заимствование строки, переданной в `rt_string_bytes_view` / `string.bytes()`; вернуть или сохранить view дольше строки — `SEM3139`.
