...
собственное значение. Поле с `float` произвольной точности пересекает вместе с
ним: счётчик за таким значением небезопасен между шардами, поэтому компилятор
делает каждый счётный блок, до которого значение дотягивается, приватным перед
отправкой — клонируя тот, что ещё держит сосед на этом шарде. `int` и `uint`
пока не считаются по ссылкам, и с них не спрашивается ничего.

Не пересекает по-прежнему то, чью счётную память не достаёт обход по
собственным байтам значения: таблица `Map`, кольцо канала, слот результата
задачи. Такое отвергается по имени, и отказ называет, что именно нашёл.

Тип без `@copy` дублируется через реализацию `__clone`. Для всей программы
выбирается ровно одна реализация, и чтобы она была доступна из другого модуля
или из обобщения, объявленного в другом месте, её нужно объявить `pub` — см.
§6.8 Clone Protocol в [LANGUAGE.ru.md](LANGUAGE.ru.md).
