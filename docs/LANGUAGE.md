# Surge Language Specification (Draft 9)
...
* **Struct:** `type Person = { age:int, name:string, @readonly weight:float }`.
  ...
  * Struct literals without an inline type require an unambiguous expected struct type. For unions like `Erring<T, Error>`, write `Error { ... }` or add a binding annotation (e.g., `let e: Error = { message = "bad", code = 1:uint };`). A struct literal that nothing gives a type -- no binding annotation, no function result at `return`, no declared field, array element or tuple element type -- is a compile error (`SemaLiteralNeedsType`, SEM3219), and so is an empty array literal `[]` with no expected array type; there is no anonymous record type. A function argument does not supply the expected type: write `Point { ... }` there.
...
- Borrowed results: `@return_source` on a parameter (or a function-type
  ...
  `docs/ATTRIBUTES.md` and `RUNTIME_V2.md`, "Borrowed Results And Normal Block Exit".
...
- `spawn on dst { ... }` is the remote-spawn form. `dst` must be a `Placement`
  value (`pool`, `distributed`, `shard(id)`, or a computed `Placement`). The
  block produces its value with `ret`, returns `far Task<T>`, and infers the
  enclosing function's crossing effect.
...
`on dst { ... }` runs a crossing block at a placement destination and returns
`TaskResult<T>`. Like `spawn on`, `dst` must be a `Placement` value or an
accepted remote handle destination. The block uses `ret`, not `return`, to
produce the block value.

The semantic effect is inferred. There is no `crosses` marker:
...
Crossing captures obey the shard-movement contract:

* Copy values may cross by copy.
* Owned user values must have a valid `@shard_movable` type.
...
backend-unavailable diagnostics instead of executing or panicking.
...
`far Task<T>` exposes remote task operations with the same names:
...
These calls consume the remote handle and infer the enclosing function's
crossing effect.
...
- Timers are driven by the async runtime. The VM uses virtual time by default
  (real time via `surge run --real-time`); native/LLVM use real time.
...
- a block expression inside a body keeps its own `ret` — `let v: int = { ret 5; };`
  ...
  `Success(v) => { ret v; }` gives the ARM its value, not the body's. A body
  therefore has no early exit from inside an arm (owner ruling 2026-08-26); it
  leaves through a flag the arm sets and the body reads after the compare —
...
let p3 = Point2D { x = 1.0, y = 2.0 } to Point3D;
```
...
```sg
// @waits_on example for a may-wait receive (parks the task)
type MessageQueue<T> = {
  items: T[],
  not_empty: Condition,
  lock: Mutex
