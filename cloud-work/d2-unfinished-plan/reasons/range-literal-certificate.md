# range literal lacks its original builtin constructor certificate
...
- **`internal/sema/return_origin_range.go:88`.** A range literal whose constructor is not the certified builtin range constructor. In the copy, the constructor is the copy's own declaration.

## Corpus examples

Each copy program analyses the whole copied directory, so the same row appears in all ten copies.

1. **`testdata/golden/core_stdlib/base.sg`**, with a row at `core_stdlib/format.sg:87`, `out = out + fmt[[seg_start..i]];`.
2. **`testdata/golden/core_stdlib/string.sg`**, with a row at `core_stdlib/format.sg:112`, the same expression.
