...
- **`internal/sema/return_origin_tags.go:97`.** The tag symbol at a constructor call does not match its original declaration by owner, name and canonical signature identity.
...
2. **`testdata/golden/core_stdlib/string.sg`**, with a row at `core_stdlib/array.sg:201`, `return Some(i to uint);`.

## Transfer, size and risk

Same as `captured-binding.md`: it stands at the same 8 sites when the real `core/` is the root (measured), and N-CORE-ROOT-TAGS would answer both.
