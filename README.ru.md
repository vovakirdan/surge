...
>
> * установщик намеренно маленький и поддерживает Linux x86_64, а также экспериментальные macOS x86_64/arm64,
...
Установка релиза Surge поддерживается на **Linux x86_64** и экспериментально на **macOS x86_64/arm64**.
Native/LLVM backend всё ещё в первую очередь тестируется на Linux x86_64.
На Apple Silicon (ARM64) есть известные проблемы с ABI (layout) в LLVM-сборках.
...
* установка релиза: `curl` или `wget`, `tar`
* сборка из исходников: `go >= 1.25.1`
...
2. Или соберите Surge из исходников:
   ...
   Опциональная системная установка из source checkout:
...
Только sendable-значения (обычно `own T`) могут пересекать границы задач, поэтому конкурентность остаётся корректной без необходимости держать доказатель теорем в голове. `@local spawn` — явный выход для захватов `@nosend`.
...
* чёткое владение между задачами (пересекают границу только sendable-значения),
...
Native/LLVM используют многопоточный исполнитель; VM-бэкенд однопоточный и детерминированный по дизайну. Задачи — это просто значения `Task<T>`; `.await()` — это метод, а не ключевое слово; блоки `async { ... }` обеспечивают структурированную конкурентность, ожидая каждую запущенную задачу. Если вам нужно уступить управление в CPU-интенсивном цикле, `checkpoint().await()` к вашим услугам вместо надежды на вытеснение. OS-блокирующая работа должна идти через `blocking { ... }` (только native/LLVM).
...
Лайфтаймы заимствований лексические, borrow checker говорит вам, где конфликт, и только sendable-значения пересекают границы worker-потоков или задач. Когда вам нужно завершить заимствование раньше, есть буквальный оператор `@drop expr;` вместо ритуальных заклинаний.

### **Структурированная конкурентность**

Асинхронный код не должен контрабандой попадать в память.

`async fn` возвращает `Task<T>`, `.await()` явный, `spawn` возвращает хэндл, который вы должны либо дождаться, либо сохранить. Никаких блуждающих задач, утекающих в пустоту. Исполнитель кооперативный; на native/LLVM он работает на нескольких воркерах, а VM остаётся однопоточной. OS-блокировка допускается только через `blocking { ... }`.
...
Часть атрибутов реально проверяется (например, `@guarded_by`, `@nonblocking`, порядок блокировок, утечки задач), а другие вроде `@pure` пока лишь разбираются. Всё это выводится через `surge diag` с файлами трассировки, которые можно загрузить в Chrome Trace Viewer, когда захочется поспелеологировать.
...
* native/LLVM-бэкенд с MT-исполнителем,
